...
```bash
curl -sSL https://raw.githubusercontent.com/Shirtiny/sub2api/custom-prod/deploy/install.sh | sudo bash
```
...
# Uninstall
curl -sSL https://raw.githubusercontent.com/Shirtiny/sub2api/custom-prod/deploy/install.sh | sudo bash -s -- uninstall -y
```
...
curl -sSL https://raw.githubusercontent.com/Shirtiny/sub2api/custom-prod/deploy/docker-deploy.sh | bash
...
git clone https://github.com/Shirtiny/sub2api.git
cd sub2api/deploy
...
Sub2API supports Grok subscription accounts through xAI OAuth, direct xAI API keys, and API-key account-pool passthrough to an OpenAI-compatible gateway such as Aether.

### Supported Scope

- Platform name: `grok`
- Account types: OAuth subscription accounts and API Key accounts. With `credentials.pool_mode=true`, the API Key account's `base_url` is treated as the Aether-compatible pool gateway.
...
- Initial text models: `grok-4.5`, `grok-4.3`, `grok-build-0.1`, `grok-composer-2.5-fast`, `grok-4.20-0309-reasoning`, `grok-4.20-0309-non-reasoning`, and `grok-4.20-multi-agent-0309`
...
<a href="https://star-history.com/#Shirtiny/sub2api&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=Shirtiny/sub2api&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=Shirtiny/sub2api&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=Shirtiny/sub2api&type=Date" />
 </picture>
</a>

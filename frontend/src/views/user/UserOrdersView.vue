      ...
      <OrderTable :orders="orders" :loading="loading">
        <template #actions="{ row }">
          <div class="flex items-center gap-2">
            ...
            <!-- Temporarily hide refund action on user orders page. Keep code commented for future restore. -->
            <!--
      ...
      <div v-if="refundTarget" class="space-y-4">
        <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-800">
          <div class="flex justify-between text-sm">
            ...
            <span class="font-mono text-content-primary">#{{ refundTarget.id }}</span>
          </div>
          <div class="mt-2 flex justify-between text-sm">
            <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.amount') }}</span>
            <span class="text-content-primary">${{ refundTarget.amount.toFixed(2) }}</span>
          </div>
        </div>
        <div>

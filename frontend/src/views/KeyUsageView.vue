  ...
  <div class="relative flex min-h-screen flex-col bg-gray-50 dark:bg-dark-950">
    ...
    <header class="relative z-20 px-6 py-4">
      <nav class="mx-auto flex max-w-6xl items-center justify-between">
        <router-link to="/home" class="flex items-center gap-3">
          ...
          <span class="text-lg font-semibold tracking-tight text-content-primary">{{ siteName }}</span>
        </router-link>
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-6 py-12">
      ...
      <div class="text-center mb-12">
        <h1 class="text-3xl sm:text-4xl font-bold tracking-tight mb-3 text-content-primary">
          {{ t('keyUsage.title') }}
        </h1>
        <p class="text-content-tertiary text-base max-w-md mx-auto">
          {{ t('keyUsage.subtitle') }}
        </p>
      </div>
      ...
      <div class="max-w-xl mx-auto mb-14">
        <div class="flex gap-3">
          <div class="flex-1 relative">
            <div class="absolute left-4 top-1/2 -translate-y-1/2 text-content-tertiary">
        ...
        <div v-if="showDatePicker" class="mt-4">
          <div class="flex flex-wrap items-center gap-2 justify-center">
            <span class="text-xs text-content-tertiary">{{ t('keyUsage.dateRange') }}</span>
            <button
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          <div v-if="statusInfo" class="fade-up flex items-center justify-center mb-2">
            <div class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-gray-200 bg-white/90 shadow-sm backdrop-blur-sm dark:border-dark-700 dark:bg-dark-900/90">
              ...
              ></span>
              <span class="text-sm font-medium text-content-primary">{{ statusInfo.label }}</span>
              <span class="text-xs text-content-tertiary">|</span>
              <span class="text-xs text-content-tertiary">{{ statusInfo.statusText }}</span>
            </div>
          </div>
          ...
          <div v-if="ringItems.length > 0" :class="ringGridClass">
            ...
            >
              <div class="flex items-center justify-between mb-6">
                <h3 class="text-sm font-semibold uppercase tracking-wider text-content-tertiary">
                  {{ ring.title }}
                </h3>
                <!-- Clock icon -->
                <svg v-if="ring.iconType === 'clock'" class="w-5 h-5 text-content-tertiary" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                </svg>
                <!-- Calendar icon -->
                <svg v-else-if="ring.iconType === 'calendar'" class="w-5 h-5 text-content-tertiary" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
                </svg>
                <!-- Dollar icon -->
                <svg v-else class="w-5 h-5 text-content-tertiary" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div class="flex justify-center">
                <div class="relative">
                  ...
                  <div class="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <template v-else>
                      <span class="text-3xl font-bold tabular-nums text-content-primary">
                        {{ displayPcts[i] ?? 0 }}%
                      </span>
                      <span class="text-xs text-content-tertiary mt-0.5">{{ t('keyUsage.used') }}</span>
                      <span
          ...
          >
            <div class="px-8 py-5 border-b border-stroke-default">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.detailInfo') }}</h3>
            </div>
            <div class="divide-y divide-gray-100 dark:divide-dark-800">
              ...
              >
                ...
                <span class="text-sm font-semibold tabular-nums" :class="row.valueClass || 'text-content-primary'">
                  {{ row.value }}
                </span>
              </div>
            </div>
          ...
          >
            <div class="px-8 py-5 border-b border-stroke-default">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.tokenStats') }}</h3>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-px bg-gray-100 dark:bg-dark-800">
              ...
              >
                <div class="text-xs text-content-tertiary mb-1">{{ cell.label }}</div>
                <div class="text-sm font-semibold tabular-nums text-content-primary">{{ cell.value }}</div>
              </div>
            </div>
          </div>
          ...
          >
            <div class="flex flex-col gap-3 px-8 py-5 border-b border-stroke-default sm:flex-row sm:items-center sm:justify-between">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.dailyDetail') }}</h3>
              ...
              <table class="w-full">
                <thead>
                  <tr class="border-b border-gray-200 bg-gray-50 dark:border-dark-700 dark:bg-dark-950">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.date') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.cacheWriteTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                  ...
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-content-primary">{{ row.date }}</td>
                    ...
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-content-primary">{{ usd(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div v-else class="px-8 py-8 text-center text-sm text-content-tertiary">
          ...
          >
            <div class="px-8 py-5 border-b border-stroke-default">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.modelStats') }}</h3>
            </div>
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-gray-200 bg-gray-50 dark:border-dark-700 dark:bg-dark-950">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.model') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.cacheCreationTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.totalTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-content-tertiary">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    class="border-b border-gray-100 last:border-b-0 dark:border-dark-800"
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-content-primary">{{ m.model || '-' }}</td>
                    ...
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-content-primary">{{ usd(m.actual_cost != null ? m.actual_cost : m.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>

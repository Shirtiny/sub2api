...
The story has a separate pause/resume button revealed on hover/focus (remaining visible when paused), outside the home link. The fallback mark itself is the control. Both support keyboard activation and background-tab pause without overriding a manual pause. Reduced motion removes all animation nodes and restores the full configured name with a static steam accent, never a half-transformed letter; the fallback retains a complete static Codex. Changing a story-enabled name recalculates targets and restarts the native clock. Below 360 px header actions wrap to avoid squeezing the name.

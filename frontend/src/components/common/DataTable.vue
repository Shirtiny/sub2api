<template>
      ...
      <div v-for="i in 5" :key="i" class="rounded-lg border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-900">
        <div class="space-y-3">
          <div v-for="column in dataColumns" :key="column.key" class="flex justify-between">
            <div class="h-4 w-20 animate-pulse rounded bg-stroke-subtle"></div>
            <div class="h-4 w-32 animate-pulse rounded bg-stroke-subtle"></div>
          </div>
          <div v-if="hasActionsColumn" class="border-t border-gray-200 pt-3 dark:border-dark-700">
            <div class="h-8 w-full animate-pulse rounded bg-stroke-subtle"></div>
          </div>
        </div>
      </div>
    </template>
        ...
        <slot name="empty">
          <div class="flex flex-col items-center">
            <Icon
              name="inbox"
              size="xl"
              class="mb-4 h-12 w-12 text-content-tertiary"
            />
            <p class="text-lg font-medium text-content-primary">
              {{ t('empty.noData') }}
            </p>
          </div>
        </slot>
    ...
    <template v-else>
      ...
      >
        <div class="space-y-3">
          ...
          >
            <span class="text-xs font-medium uppercase tracking-wider text-content-tertiary">
              {{ column.label }}
            </span>
            <div class="text-right text-sm text-content-primary">
  ...
  >
    <table class="w-full min-w-max divide-y divide-stroke-default">
      <thead class="table-header bg-surface-secondary">
        <tr>
          <th
            ...
            scope="col"
            :class="[
              'sticky-header-cell py-3 text-left text-xs font-medium uppercase tracking-wider text-content-tertiary',
              getAdaptivePaddingClass(),
              { 'cursor-pointer hover:bg-surface-hover': column.sortable },
          ...
          >
            ...
            >
              <div class="flex items-center space-x-1">
                ...
                <span v-if="column.sortable" class="text-content-tertiary">
                  <svg
      ...
      <tbody class="table-body divide-y divide-stroke-default bg-surface-card">
        <!-- Loading skeleton -->
        <tr v-if="loading" v-for="i in 5" :key="i">
          <td v-for="column in columns" :key="column.key" :class="['whitespace-nowrap py-4', getAdaptivePaddingClass()]">
            <div class="animate-pulse">
              <div class="h-4 w-3/4 rounded bg-stroke-subtle"></div>
            </div>
          </td>
        </tr>
        ...
        <tr v-else-if="!data || data.length === 0">
          <td
            ...
            :class="['py-12 text-center text-content-tertiary', getAdaptivePaddingClass()]"
          >
            <slot name="empty">
              <div class="flex flex-col items-center">
                <Icon
        ...
        <template v-else>
          ...
          >
            <td
              ...
              :key="column.key"
              :class="[
                'whitespace-nowrap py-4 text-sm text-content-primary',
                getAdaptivePaddingClass(),

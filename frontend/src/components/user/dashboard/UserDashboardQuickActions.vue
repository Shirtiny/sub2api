<template>
  <div class="card">
    <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
      <h2 class="text-lg font-semibold text-content-primary">{{ t('dashboard.quickActions') }}</h2>
    </div>
    <div class="space-y-3 p-4">
      <button @click="router.push('/keys')" class="group flex w-full items-center gap-4 rounded-xl bg-gray-50 p-4 text-left transition-all duration-200 hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
        ...
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-content-primary">{{ t('dashboard.createApiKey') }}</p>
          <p class="text-xs text-content-tertiary">{{ t('dashboard.generateNewKey') }}</p>
        </div>
        <Icon
          name="chevronRight"
          size="md"
      ...
      <button @click="router.push('/usage')" class="group flex w-full items-center gap-4 rounded-xl bg-gray-50 p-4 text-left transition-all duration-200 hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
        ...
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-content-primary">{{ t('dashboard.viewUsage') }}</p>
          <p class="text-xs text-content-tertiary">{{ t('dashboard.checkDetailedLogs') }}</p>
        </div>
        <Icon
          name="chevronRight"
          size="md"
      ...
      <button @click="router.push('/redeem')" class="group flex w-full items-center gap-4 rounded-xl bg-gray-50 p-4 text-left transition-all duration-200 hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
        ...
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-content-primary">{{ t('dashboard.redeemCode') }}</p>
          <p class="text-xs text-content-tertiary">{{ t('dashboard.addBalanceWithCode') }}</p>
        </div>
        <Icon
          name="chevronRight"
          size="md"

<template>
  <div class="group/usage relative text-sm">
    <div class="flex items-center gap-1.5">
      ...
      <span class="font-medium text-content-primary">${{ today.toFixed(4) }}</span>
      <Icon
        v-if="hasBreakdown"
        name="infoCircle"
        size="xs"
    ...
    <div class="mt-0.5 flex items-center gap-1.5">
      ...
      <span class="font-medium text-content-primary">${{ total.toFixed(4) }}</span>
    </div>
...
// 与 UserDashboardStats 保持一致：长期总量与现存平台明细的差值归为历史/未分类。

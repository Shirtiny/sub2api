<template>
  <div class="space-y-4">
          ...
          <div v-if="paidOrder" class="w-full rounded-xl bg-gray-50 p-4 dark:bg-dark-800">
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                ...
                <span class="font-medium text-content-primary">#{{ paidOrder.id }}</span>
              </div>
              <div v-if="paidOrder.out_trade_no" class="flex justify-between">
                <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.orderNo') }}</span>
                <span class="font-medium text-content-primary">{{ paidOrder.out_trade_no }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.amount') }}</span>
                <span class="font-medium text-content-primary">{{ paidOrder.order_type === 'balance' ? '$' + paidOrder.amount.toFixed(2) : formatGatewayAmount(paidOrder.amount) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') }}</span>
                <span class="font-medium text-content-primary">{{ formatGatewayAmount(paidOrder.pay_amount) }}</span>
              </div>
            </div>
          </div>
    ...
    <template v-else-if="outcome === 'cancelled'">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          <div class="flex h-16 w-16 items-center justify-center rounded-full bg-surface-hover">
          ...
          <p class="text-lg font-bold text-content-primary">{{ t('payment.qr.cancelled') }}</p>
    ...
    <template v-else-if="outcome === 'expired'">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <p class="text-lg font-bold text-content-primary">{{ t('payment.qr.expired') }}</p>
    ...
    <template v-else-if="qrUrl">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4">
          <p class="text-lg font-semibold text-content-primary">{{ scanTitle }}</p>
      ...
      <div class="card p-4 text-center">
        ...
        <p class="mt-1 text-2xl font-bold tabular-nums text-content-primary">{{ countdownDisplay }}</p>
...
async function pollStatus() {
  ...
  if (isSuccessStatus(order.status)) {
    ...
    setOutcome('success')
    emit('success', order)
  } else if (order.status === 'CANCELLED' || order.status === 'PRESALE_CANCELLED') {
    cleanup()

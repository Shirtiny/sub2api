    ...
    <div class="billing-options">
      <article v-for="(plan, index) in plans" :key="plan" class="billing-option" :aria-labelledby="`billing-${plan}-title`" data-reveal :style="{ '--reveal-delay': `${280 + index * 140}ms` }">
        ...
        <div class="billing-metric">
          <strong>{{ plan === 'subscription' ? '1' : t('home.landing.billing.balance.availability') }}</strong>
          <span v-if="plan === 'subscription'">{{ t('home.landing.billing.subscription.unit') }}</span>
        </div>
        <p v-if="plan === 'subscription'" class="billing-caption">{{ t('home.landing.billing.subscription.caption') }}</p>
        <dl class="billing-facts">
          <div v-for="fact in facts[plan]" :key="fact">
...
const facts = {
  subscription: ['access', 'period', 'pricing'],
  balance: ['access', 'period']
} as const
</script>
...
.billing-option { min-width: 0; padding: clamp(24px, 3vw, 40px); border: 1px solid var(--cafe-line); border-radius: 12px; background: var(--cafe-page); }
...
.billing-metric strong { font: 500 clamp(40px, 4vw, 60px)/1.1 Georgia, serif; letter-spacing: -.04em; }
.billing-metric span { font-size: 14px; }
.billing-caption { margin-top: 8px; font-size: 12px; color: var(--cafe-muted); }
.billing-facts { margin-top: 24px; }
...
@media (min-width: 1024px) and (min-height: 700px) {
  ...
  .billing-metric strong { font-size: clamp(40px, 5dvh, 54px); }

...
describe('risk control locale copy', () => {
  ...
  it('documents that snapshot capture strictly follows the switch', () => {
    expect(zh.admin.riskControl.requestControl.requestSnapshotHint).toContain('开关开启时')
    expect(zh.admin.riskControl.requestControl.requestSnapshotHint).toContain('关闭后不采集任何新快照')
    expect(en.admin.riskControl.requestControl.requestSnapshotHint).toContain('When enabled')
    expect(en.admin.riskControl.requestControl.requestSnapshotHint).toContain('When disabled, no new snapshots are captured')
  })
})

import presaleNotice from './presaleNotice.en'
...
export default {
  ...
  home: {
    ...
    tags: {
      subscriptionToApi: 'Stable Subscription',
    ...
    features: {
      ...
      multiAccountDesc: '100% Team & Pro Access. Zero Free Tiers. Engineered for Maximum Stability and Uninterrupted Workflow.',
  ...
  membership: {
    ...
    totalRecharged: 'Membership points: {amount}',
    progressTo: 'Progress to LV.{level}',
    highest: 'Highest level achieved',
    highestHint: 'LV.5 is the highest membership level.',
    ...
    cafeCoupon: {
      label: 'Café coupon:',
      labelText: 'Café coupon',
      claim: 'Claim',
      claiming: 'Claiming...',
      claimed: 'Claimed',
      view: 'View',
      alreadyClaimed: 'Café coupon already claimed for this period',
      disabled: 'Disabled',
      tip: 'Café coupons can be used for discounts during recharge or subscription payments, and specific benefits depend on the current membership level.',
      remainingDays: 'Claim again in {days} day(s)',
      nextClaimLater: 'Available again next period',
      claimDiscountHint: 'One coupon every {period}, current level discount {value}%',
      claimCashHint: 'One coupon every {period}, current level deducts ¥{value}',
      benefitDiscount: '{value}% off',
      benefitCash: 'Redeem ¥{value}',
      period: {
        day: 'day',
        week: 'week',
        month: '30 days'
      ...
      nonTransferableHint: ' This coupon can only be used by this account.',
  ...
  keys: {
    ...
    endpoints: {
      ...
      latencyUnavailable: 'Latency check failed',
    },
    ...
    keyRotatedSuccess: 'API key rotated. Copy the new key now.',
    rotate: 'Rotate',
    rotateKey: 'Rotate Key',
    rotateConfirmMessage: 'Rotate "{name}"? The old key will stop working immediately, and the full new key is shown only once.',
    failedToRotate: 'Failed to rotate API key',
    keyOnlyShownOnceShort: 'The full key is only shown when it is created or rotated',
    oneTimeKey: {
      ...
      warning: 'This key appears only once here.',
      copy: 'Copy key',
      closeConfirmTitle: 'Confirm before closing',
      closeConfirmMessage: 'After closing, you will not be able to view the full key again. Close this window?',
      closeConfirmConfirm: 'Close window'
    },
    ...
    virtualCustomEntitlementHint: 'This key is bound to the source group, but currently uses the {multiplier} custom subscription entitlement from {name}.',
  ...
  availableChannels: {
    catalog: {
      ...
      contextUpTo: 'Up to {max}',
      contextAbove: 'Over {min}',
      contextBetween: 'Over {min}, up to {max}',
      contextAny: 'All contexts',
      ...
      listPrice: 'List',
      multiplierHint: 'Model price = list price × this multiplier',
      priceDetails: 'View details',
      perRequestUnit: 'USD / request',
      groups: 'Available groups',
      groupMultiplier: 'Group rate (including your personal rate, if configured)',
      noResults: 'No matching models. Try another search.',
    },
  ...
  profile: {
    ...
    concurrencyRules: {
      ...
      subscription: 'Active subscriptions determine your limit. Multiple subscriptions use the highest limit, not the sum.',
      balance: 'Without an active subscription, your current account balance determines the limit:',
      interval: '{min} ≤ balance < {max}',
      unbounded: 'Balance ≥ {min} (no upper bound)',
      note: 'Pending presales do not count. All API keys share this limit, which adjusts with your balance and subscription status.',
    },
    rpmLimit: 'RPM Limit',
  ...
  admin: {
    ...
    users: {
      ...
      concurrencyRules: {
        title: 'Concurrency rules',
        description: 'Global rules for users without an active subscription, based on their current account balance ($), not the recharge amount in RMB. Active subscriptions still take priority; multiple subscriptions use the highest limit, not the sum. Saved rules apply to new requests and do not cancel requests already in flight.',
        coverage: 'Each tier includes its lower bound and excludes its upper bound, which is derived from the next tier. Start at $0 and increase lower bounds to avoid gaps or overlaps. The highest tier has no upper bound.',
        tier: 'Tier {number}',
        minBalance: 'Balance from ($, inclusive)',
        upperBalance: 'Balance to ($, exclusive)',
        concurrency: 'Concurrent requests',
        firstFixed: 'The first lower bound is fixed at $0.',
        noUpperBound: 'No upper bound',
        pendingBound: 'Enter a valid next lower bound',
        addTier: 'Add tier',
        removeTier: 'Remove tier',
        removeTierLabel: 'Remove tier {number}',
        tierCount: '{count} / 20 tiers (at least 1)',
        retry: 'Retry loading',
        loadFailed: 'Could not load concurrency rules. Saving is disabled until loading succeeds.',
        saveFailed: 'Could not save concurrency rules. Your edits are preserved; please retry.',
        saved: 'Concurrency rules saved',
        validation: {
          summary: 'Use 1–20 continuous tiers and correct the highlighted values before saving.',
          balance: 'Enter a finite balance between $0 and $1,000,000,000,000.',
          order: 'Start at $0. Each subsequent lower bound must be strictly greater than the previous one.',
          concurrency: 'Enter a whole number from 1 to 1000.'
        }
      },
      ...
      currentMembershipLevel: 'Current Membership Level',
      membershipPoints: 'Membership Points',
      membershipPointsUnit: 'pts',
      membershipPointsTip: 'Click to edit membership points',
      newMembershipPoints: 'New Membership Points',
      newMembershipLevel: 'New Membership Level',
      setPoints: 'Set Points',
      addPoints: 'Add Points',
      subtractPoints: 'Subtract Points',
      pointsRequired: 'Please enter valid points',
      insufficientMembershipPoints: 'Insufficient membership points',
      ...
      createdAt: 'Created',
      totalRecharged: 'Membership Points',
      roles: {
        admin: 'Admin',
        user: 'User'
      },
    ...
    groups: {
      ...
      customSubscription: {
        badge: 'Custom',
        title: 'Custom subscription info',
        ownerUser: 'Owner user',
        sourcePlan: 'Source plan',
        sourceGroup: 'Source group',
        multiplier: 'Current multiplier',
        multiplierHint: 'After saving, the backend recalculates daily/weekly/monthly limits from the source group without changing historical order amounts.',
        multiplierInvalid: 'Custom multiplier must be at least 1.',
      },
      subscription: {
      ...
      videoPricing: {
        ...
        description: 'Configure Grok video generation per-second prices. Leave empty to use the default xAI rate card.',
        independentMultiplier: 'Use independent video multiplier',
        videoMultiplier: 'Video multiplier',
        modeHint: 'Default billing uses per-second price × duration × effective group multiplier. Independent mode uses the video multiplier.',
    ...
    channels: {
      ...
      form: {
        ...
        billingMode: 'Billing Mode',
        listPrices: 'List prices & multiplier',
        priceMultiplier: 'Model multiplier',
        priceMultiplierHint: 'All prices below are list prices, including cache and tiers. Model price = list price × multiplier; group or personal rates apply separately. Empty prices follow existing fallback rules.',
        invalidPriceMultiplier: 'Model multiplier must be greater than 0 and at most 1000',
        defaultPrices: 'Default list prices (outside configured intervals)',
        inputPrice: 'Input',
        outputPrice: 'Output',
        cacheWritePrice: 'Cache Write',
        cacheReadPrice: 'Cache Read',
        ...
        defaultPerRequestPrice: 'Default per-request list price (fallback when no tier matches)',
        defaultImagePrice: 'Default image list price (fallback when no tier matches)',
    ...
    riskControl: {
      title: 'Risk Control',
      description: 'Configure content auditing and request control with their records',
      ...
      contentHitPolicy: 'Content audit hit policy',
      contentHitPolicyHint: 'Counts content-audit and keyword hits only; upstream Cyber blocks are excluded.',
      ...
      cyberPolicyEnabled: 'Enable upstream Cyber risk control',
      cyberPolicyEnabledHint: 'Recognize upstream 400 Cyber or cyber_policy responses and record the corresponding user in risk control.',
      cyberPolicySection: 'Upstream Cyber risk control',
      cyberPolicySectionHint: 'Cyber hits use an independent count, ban threshold, and time window instead of sharing content-audit state.',
      cyberPolicyEmail: 'Send Cyber abuse notice email',
      cyberPolicyEmailHint: 'Notify the corresponding CafeCode user through the configured email service.',
      cyberPolicyEmailMessage: 'Cyber email message',
      cyberPolicyEmailMessageHint: 'Replaces only the opening explanation sentence; leave blank to use the localized default.',
      cyberPolicyEmailMessagePlaceholder: "Your request was blocked by the upstream provider's cyber-security policy.",
      cyberPolicyAutoBan: 'Auto-ban on Cyber hits',
      cyberPolicyAutoBanHint: 'Disable the user and send a ban notice when the independent Cyber count reaches its threshold.',
      cyberPolicyBanThreshold: 'Cyber ban threshold',
      cyberPolicyViolationWindowHours: 'Cyber counting window (hours)',
      ...
      inputDetailContent: 'Input Summary / Match Context',
      ...
      requestControl: {
        ...
        protocolBlocking: 'Protocol blocking switches',
        protocolBlockingHint: 'When off, the policy still evaluates and records the expected result, but the request is observed and allowed without blocking, counting, or notifications. Anonymous Responses requests remain blocked; explicit compaction signals are observed and allowed, while a strong wire shape only supplies a session before normal policy continues.',
        blockOpenAIChat: 'Block OpenAI Chat requests',
        blockClaudeMessages: 'Block Claude Messages requests',
        blockOpenAIResponses: 'Block OpenAI Responses requests',
        requestSnapshot: 'Save request diagnostic snapshots',
        requestSnapshotHint: 'When enabled, blocked or observed requests retain redacted headers and up to 256KB of body head/tail for 3 days. When disabled, no new snapshots are captured.',
        requestSnapshotRuntime: 'Service runtime snapshot switch',
        ...
        statusFailed: 'Failed to load request control status',
        logsFailed: 'Failed to load request control records',
        scopeTitle: 'Control Scope',
        scopeHint: 'The request path uses an in-memory snapshot and refreshes after policy changes.',
        groups: 'Group scope',
        allGroups: 'All groups',
        selectedGroups: 'Selected groups only',
        noGroups: 'No groups available',
        models: 'Model scope',
        modelsPlaceholder: 'One model per line',
        globalUA: 'Global User-Agent whitelist',
        uaPlaceholder: 'One UA prefix per line; matching requests bypass control',
        uaHint: 'User-level markers are merged with the global list; they do not replace it.',
        blockStatus: 'Block status',
        blockStatusHint: 'The client uses this HTTP status; default is 403.',
        blockMessage: 'Block message',
        blockMessageHint: 'Used in the client block response and request-control notification emails.',
        notificationsTitle: 'Hit Notifications and Auto-Ban',
        notificationsHint: 'Hits are counted asynchronously; the same user is counted again only after a gap longer than 5 minutes.',
        emailOnHit: 'Email on hit',
        emailOnHitHint: 'Send a risk-control notice for every counted hit; auto-ban notices are always sent.',
        autoBan: 'Auto-ban user',
        autoBanHint: 'Disable the account, invalidate its auth cache, and send a ban notice when the threshold is reached.',
        banThreshold: 'Ban threshold',
        violationWindowHours: 'Violation window (hours)',
        violationWindowHint: 'A new counted hit requires more than 5 minutes since the user’s previous hit request.',
        violationCount: '{count} counted hits',
        notCounted: 'Not added to count',
        emailSent: 'Email sent',
        hitEmailSent: 'Hit notice sent',
        banEmailSent: 'Ban notice sent',
        autoBanned: 'Auto-banned',
        usersTitle: 'User Scope and Exceptions',
        usersHint: 'All users participate by default; when disabled, only users marked as participating are checked.',
        searchUser: 'Search by email, username, or UID',
        searching: 'Searching',
        noUsersFound: 'No users found',
        selectedUser: 'Selected: {user}',
        userSearchFailed: 'Failed to search users',
        userID: 'User ID',
        userUAPlaceholder: 'User UA prefixes, separated by commas or new lines',
        addUser: 'Add user',
        updateUser: 'Update user',
        editUserUA: 'Edit user UA whitelist',
        allUsersOn: 'All users participate unless excluded',
        onlyConfiguredUsers: 'Only configured participating users are checked',
        ruleCount: '{count} user rules',
        participate: 'Participate',
        userUA: 'User UA whitelist',
        removeUser: 'Remove user rule',
        noUserRules: 'No user rules',
        logsTitle: 'Control Records',
        logsHint: 'Records are aggregated by user and request-format fingerprint with count and first/last seen time. When the snapshot switch is enabled, blocked and observed requests are captured; when disabled, no new snapshots are stored. Details refresh at most once per 15 minutes; body snapshots are capped at 256KB and kept for 3 days. Base records are kept for 30 days.',
        refresh: 'Refresh records',
        search: 'Search user, UA, reason, or request ID',
        applyFilters: 'Apply filters',
        allActions: 'All results',
        blocked: 'Blocked',
        observed: 'Observed',
        ...
        loggedAt: 'TLS observation',
        tlsUnavailable: 'Not captured',
        detail: 'Detail',
        viewDetail: 'View request detail',
        detailTitle: 'Request Control Detail',
        detailFailed: 'Failed to load request detail',
        requestID: 'Request ID',
        eventCount: '{count} aggregated events',
        singleEvent: '1 occurrence',
        eventCountLabel: 'Aggregated events',
        firstSeen: 'First seen',
        lastSeen: 'Last seen',
        requestHeaders: 'Request headers (redacted)',
        requestHeadersSummary: 'Request header summary (legacy compatible)',
        fullRequestHeaders: 'Full request headers (credentials redacted)',
        requestBodySnapshot: 'Request body diagnostic snapshot',
        requestMethod: 'Request method',
        requestHost: 'Request host',
        requestPath: 'Request path and query',
        clientAddress: 'Client address',
        bodySize: 'Original body size',
        capturedSize: 'Captured body size',
        snapshotCapturedAt: 'Snapshot captured at',
        bodyTruncatedHint: 'The body exceeded the snapshot limit. Head and tail totaling {captured} were saved from {total}; use SHA-256 to verify the original content.',
        snapshotUnavailable: 'No request snapshot is available. The switch may have been disabled at the time, the record may be historical, its snapshot may have exceeded the 3-day retention period, or capture/storage may have degraded.',
        snapshotUnavailableReason: 'Capture state: {reason}',
        requestBodyMetadata: 'Request body metadata',
      ...
      keywordWhitelist: 'Keyword whitelist',
      keywordWhitelistPlaceholder: 'One whitelisted keyword per line, e.g.:\nnormal business term\nproper noun',
      keywordWhitelistCount: '{count} whitelist keywords configured',
      keywordWhitelistHint: 'Whitelist has priority over the built-in filter and blocked keywords. Whitelist hits skip keyword blocking only; API moderation still follows the selected strategy.',
      ...
      builtInFilterEnabled: 'Enable built-in filter library',
      builtInFilterEnabledDesc: 'When enabled, the built-in filter library and the keywords configured here both apply. When disabled, only configured keywords apply.',
      builtInFilterCategories: 'Built-in library categories',
      builtInFilterCategoriesDesc: 'Only built-in terms in selected categories are matched. All categories are selected by default.',
      builtInFilterLevels: 'Built-in library levels',
      builtInFilterLevelsDesc: 'Only built-in terms at selected levels are matched. By default, only high is selected.',
      builtInFilterSelectedCount: '{count} selected',
      builtInFilterCategoryPolitical: 'Political',
      builtInFilterCategoryPornographic: 'Pornographic',
      builtInFilterCategoryViolence: 'Violence',
      builtInFilterCategoryAbuse: 'Abuse',
      builtInFilterCategoryAd: 'Ad redirect',
      builtInFilterCategoryIllegal: 'Illegal',
      builtInFilterCategoryOther: 'Other',
      builtInFilterLevelLow: 'Low',
      builtInFilterLevelMedium: 'Medium',
      builtInFilterLevelHigh: 'High',
      builtInFilterLevelCritical: 'Critical',
      overview: {
        status: 'Status',
        enabled: 'Enabled',
        disabled: 'Disabled',
    ...
    subscriptions: {
      ...
      resetCount: 'Resets remaining',
      setResetCount: 'Set reset count',
      setResetCountTitle: 'Set subscription reset count',
      resetCountLabel: 'Resets remaining',
      resetCountHint: 'Number of self-service daily/weekly quota resets (0-{max}).',
      settingResetCount: 'Saving...',
      resetCountUpdated: 'Subscription reset count updated',
      failedToSetResetCount: 'Failed to set subscription reset count',
      bulkSetResetCount: 'Set reset counts in bulk',
      bulkSetResetCountSelected: 'Set {count} selected',
      bulkSetResetCountSelectedMessage: 'Set the same remaining reset count for the {count} selected subscriptions; plans without a daily/weekly window are skipped when the value is above 0 (use 0 to clear stale allowances).',
      bulkSetResetCountTitle: 'Set subscription reset counts in bulk',
      bulkSetResetCountMessage: 'Set the same remaining reset count for all active subscriptions; plans without a daily/weekly window are skipped when the value is above 0 (use 0 to clear stale allowances).',
      bulkResetCountSuccess: 'Updated {updated} subscriptions, skipped {skipped}',
      selectAllForResetCount: 'Select all subscriptions for reset-count update',
      selectForResetCount: 'Select subscription for reset-count update',
      resetCountErrors: {
        INVALID_QUOTA_RESET_COUNT: 'Reset count must be an integer between 0 and 1000.',
        QUOTA_RESET_DISABLED: 'This subscription does not support daily/weekly self-service reset counts.',
        QUOTA_RESET_UNAVAILABLE: 'Reset-count management is temporarily unavailable. Try again later.',
        QUOTA_RESET_CONFLICT: 'The subscription changed. Refresh and try again.',
        TOO_MANY_SUBSCRIPTIONS: 'At most 1000 subscriptions can be updated at once.',
      ...
      quotaResetSuccess: 'Quota reset successfully',
      failedToResetQuota: 'Failed to reset quota',
      bulkResetQuota: 'Reset All Quotas',
      bulkResetQuotaTitle: 'Reset All Subscription Quotas',
      bulkResetQuotaConfirm: 'Select the usage windows to reset. This affects every active subscription, not just the current filters or page. The selected windows are zeroed and restart from today.',
      bulkResetQuotaDaily: 'Daily usage',
      bulkResetQuotaWeekly: 'Weekly usage',
      bulkResetQuotaMonthly: 'Monthly usage (affects monthly-quota subscriptions — check with care)',
      bulkResetQuotaNoWindow: 'Select at least one usage window.',
      bulkQuotaResetSuccess: 'Reset quotas for {count} subscriptions',
      shiftWindow: 'Shift Reset Window',
      shiftWindowTitle: 'Shift Reset Windows in Bulk',
      shiftWindowWarning: 'This bulk-edits subscription window starts. It only moves the next reset time and never changes recorded usage. Double-check the filter scope before submitting.',
      shiftWindowWindows: 'Windows to move',
      shiftWindowDaily: 'Daily window',
      shiftWindowWeekly: 'Weekly window',
      shiftWindowMonthly: 'Monthly window',
      shiftWindowNoWindow: 'Select at least one window.',
      shiftWindowOffsetHours: 'Offset (hours)',
      shiftWindowOffsetHint: 'Positive delays the reset, negative brings it forward. Must be non-zero and at most {max} hours.',
      shiftWindowOffsetRequired: 'Enter a non-zero hour offset.',
      shiftWindowOffsetInteger: 'The hour offset must be a whole number.',
      shiftWindowOffsetRange: 'The hour offset must not exceed {max} in absolute value.',
      shiftWindowScope: 'Scope',
      shiftWindowScopeHint: 'The scope follows the current page filters and is not limited by pagination.',
      shiftWindowScopePlatform: 'Platform',
      shiftWindowPreview: 'Will affect {matched} subscriptions; {skipped} skipped because the window start would land in the future.',
      shiftWindowPreviewLoading: 'Calculating scope…',
      shiftWindowPreviewUnavailable: 'Pick a window and a valid hour offset to preview the affected scope.',
      shiftWindowSubmit: 'Apply Shift',
      shiftWindowSubmitting: 'Applying...',
      shiftWindowSuccess: 'Shifted reset windows for {count} subscriptions, skipped {skipped}',
      shiftWindowFailed: 'Failed to shift reset windows',
      stats: {
        openButton: 'Statistics',
        title: 'Subscription Statistics',
        detailTitle: 'Subscription Usage Detail',
        remainingToday: 'Remaining Today',
        remainingWeek: 'Remaining This Week',
        horizonCapacity: 'Upcoming Capacity',
        horizonOption: '{days} days',
        dailyLimitedCount: '{count} daily-limited subscriptions',
        weeklyLimitedCount: '{count} weekly-limited subscriptions',
        totalSubscriptions: '{count} subscriptions in total',
        byPlan: 'Breakdown by Plan',
        colPlan: 'Plan',
        colSubscriptions: 'Subs',
        colRemainingToday: 'Remaining Today',
        colRemainingWeek: 'Remaining This Week',
        colRemainingMonth: 'Remaining This Month',
        colHorizon: '{days}-day Capacity',
        ...
        weeklyUsage: 'Weekly Usage Rate',
        derived: 'derived',
        derivedDailyHint: 'This plan has no daily limit; the denominator is weekly limit ÷ 7',
        derivedWeeklyHint: 'This plan has no weekly limit; the denominator is daily limit × 7',
        historyTruncated: 'History starts at {date}. Earlier usage was purged with the usage_logs retention window and cannot be recovered.',
        historyEmpty: 'No historical usage yet. The rollup table was just created and needs time to accumulate a full cycle.',
        generatedAt: 'Generated at {time}',
        noData: 'No data',
        loadFailed: 'Failed to load subscription statistics',
        seriesLoadFailed: 'Failed to load usage detail'
      },
      failedToBulkResetQuota: 'Failed to reset all quotas',
      ...
      guide: {
        ...
        actions: {
          ...
          resetCountDesc: 'Set the remaining daily/weekly quota resets a user may perform',
          resetQuota: 'Reset Quota',
    ...
    accounts: {
      ...
      types: {
        ...
        grokApiKey: 'Direct xAI or Aether account pool',
      ...
      grok: {
        baseUrlHint: 'Use the official xAI API base URL for direct mode, or the Aether gateway base URL when account pool mode is enabled.',
        apiKeyHint: 'Enter an xAI API key for direct mode, or an Aether gateway key for account pool passthrough.'
      },
      anthropic: {
      ...
      oauth: {
        ...
        grok: {
          ...
          oauthOnlyHint: 'Use OAuth for direct Grok subscription access; use API Key for direct xAI or Aether pool passthrough.'
        },
        // Gemini specific
	        gemini: {
    ...
    promo: {
      ...
      tabs: {
        ...
        cafeCoupons: 'Café Coupons'
      },
      cafeCoupon: {
        searchPlaceholder: 'Search code or user ID...',
        allTypes: 'All Types',
        ...
        statusUpdated: 'Café coupon status updated',
        statusNotAllowed: 'Café coupons on completed paid orders cannot be restored to unused',
        failedToUpdateStatus: 'Failed to update Café coupon status',
        resetClaimPeriod: 'Reset Claim Period',
        resetClaimPeriodConfirm: 'Reset this user\'s Café coupon claim eligibility? The user\'s latest Café coupon record will be moved outside its claim cycle; if that latest record is issued and unused, it will also be voided. The user can claim again immediately.',
        claimPeriodReset: 'Claim period reset',
        failedToResetClaimPeriod: 'Failed to reset claim period',
        voidCoupon: 'Void Café Coupon',
        voidCouponConfirm: 'Are you sure you want to void this Café coupon? This action cannot be undone.',
        voided: 'Café coupon voided successfully',
        voidNotAllowed: 'Only issued unused Café coupons can be voided',
        failedToLoad: 'Failed to load Café coupons',
        failedToVoid: 'Failed to void Café coupon'
      }
    },
    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          membershipThresholds: 'Membership point thresholds',
          membershipThresholdsDesc: 'LV.1 > 20, LV.2 > 300, LV.3 > 1000, LV.4 > 3000, LV.5 > 5000.',
          rebateRate: 'Membership Rebate Rates',
          rebateRateHint: 'Default amount rebate rate by inviter membership level; LV.0 cannot invite or earn rebates, and per-user overrides take precedence. Defaults: LV.1 5%, LV.2 5%, LV.3 8%, LV.4 10%, LV.5 12%.',
          rebateRateLevel0: 'Regular / LV.0 (not eligible)',
          rebateRateLevel1: 'LV.1',
          rebateRateLevel2: 'LV.2',
          rebateRateLevel3: 'LV.3',
          rebateRateLevel4: 'LV.4',
          rebateRateLevel5: 'LV.5',
          ...
          perInviteeCapDesc: 'Set the maximum total rebate from a single invitee by inviter membership level. 0 means no rebate can be earned. Defaults: Normal 0, LV.1 100, LV.2 300, LV.3-LV.5 1000.',
          perInviteeCapLevel0: 'Normal',
          perInviteeCapLevel1: 'LV.1',
          perInviteeCapLevel2: 'LV.2',
          perInviteeCapLevel3: 'LV.3',
          perInviteeCapLevel4: 'LV.4',
          perInviteeCapLevel5: 'LV.5',
          inviteLimit: 'Membership-level invite limits',
          inviteLimitDesc: 'Set max invitees by inviter membership level. 0 = disabled/unavailable; override specific users below. Defaults: Normal 0, LV.1 1, LV.2 3, LV.3-LV.5 5.',
          inviteLimitLevel0: 'Normal',
          inviteLimitLevel1: 'LV.1',
          inviteLimitLevel2: 'LV.2',
          inviteLimitLevel3: 'LV.3',
          inviteLimitLevel4: 'LV.4',
          inviteLimitLevel5: 'LV.5',
          customUsers: {
        ...
        cafeCoupon: {
          title: 'Café Coupon',
          description: 'Configure Café coupon benefits by membership level. Defaults: LV.1 5 cash, LV.2 10 cash, LV.3 10% discount, LV.4-LV.5 15% discount; only LV.5 is transferable by default. Coupon validity is 30% of the claim period.',
          level0: 'Regular / LV.0',
          level1: 'LV.1',
          level2: 'LV.2',
          level3: 'LV.3',
          level4: 'LV.4',
          level5: 'LV.5',
          levelHint: 'When enabled, users can claim this level coupon by period.',
          type: 'Coupon Type',
          ...
          monthEndValidityTitle: 'Validity is 30% of the claim period',
          monthEndValidityHint: 'Daily claims last about 7.2 hours, weekly claims about 2.1 days, and 30-day claims last 9 days.',
          valueHint: 'Cash coupons use amount; discount coupons use a 0-100 discount percent. Value 0 makes the backend disable that level.',
        },
      },
      ...
      payment: {
        ...
        guestShopEnabledHint: 'This switch is independent from the main payment switch above. Turning it off only blocks new guest payments.',
        guestShopStripeInstance: 'Guest Checkout Stripe Instance',
        guestShopStripeInstanceHint: 'A Stripe instance disabled for the main site can still be selected. Guest checkout only reads its keys, currency, and methods without changing its status.',
        guestShopStripeNotSelected: 'Select a Stripe instance',
        guestShopStripeOption: '{name} (ID {id}) · {status}',
        guestShopNoStripeInstance: 'No Stripe instance is available. Create and fully configure one under Provider Management first.',
        guestShopSelectionRequired: 'Select a fully configured Stripe instance before enabling guest checkout.',
        guestShopOriginalStatus: 'Main-site instance status',
        guestShopOriginalEnabled: 'Enabled for main site',
        guestShopOriginalDisabled: 'Disabled for main site',
        guestShopCurrency: 'Currency',
        guestShopMethods: 'Supported methods',
        guestShopNotConfigured: 'Not configured',
  ...
  userSubscriptions: {
    ...
    resetQuota: 'Reset daily/weekly quota',
    resetQuotaConfirmTitle: 'Confirm quota reset',
    resetQuotaConfirmMessage: 'Reset the daily and weekly quota for “{name}” now? This consumes 1 self-service reset (leaving {remaining}). The action cannot be undone.',
    resetQuotaConfirm: 'Reset now',
    resetQuotaResetting: 'Resetting...',
    resetQuotaSuccess: 'Daily and weekly subscription quota reset',
    resetQuotaFailed: 'Failed to reset subscription quota',
    resetQuotaErrors: {
      INVALID_QUOTA_RESET_COUNT: 'Reset count must be an integer between 0 and 1000.',
      QUOTA_RESET_DISABLED: 'Self-service quota reset is not available for this subscription.',
      QUOTA_RESET_EXHAUSTED: 'This subscription has no reset allowances remaining.',
      QUOTA_RESET_CONFLICT: 'The subscription changed. Refresh the page and try again.',
      QUOTA_RESET_UNAVAILABLE: 'Self-service reset is temporarily unavailable. Try again later.',
      SUBSCRIPTION_EXPIRED: 'This subscription has expired.',
      SUBSCRIPTION_SUSPENDED: 'This subscription is suspended.',
      SUBSCRIPTION_NOT_STARTED: 'This subscription has not started yet.',
      SUBSCRIPTION_NOT_FOUND: 'Subscription not found.'
    },
    earlyResetErrors: {
  ...
  onboarding: {
    ...
    user: {
      ...
      keySubmit: {
        ...
        description: '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;">Click to confirm and create your API key.</p><div style="padding: 8px 12px; background: #fee2e2; border-left: 3px solid #ef4444; border-radius: 4px; font-size: 13px; margin-bottom: 12px;"><b>⚠️ Important:</b><ul style="margin: 8px 0 0 16px;"><li>Copy the key (cafepass-uid-xxxxx) immediately after creation</li><li>Key is only shown once, need to regenerate if lost</li></ul></div><p style="padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #10b981; border-radius: 4px; font-size: 13px;"><b>🚀 How to Use:</b><br/>Configure the key in any OpenAI-compatible client (like ChatBox, OpenCat, etc.) and start using!</p><p style="margin-top: 12px; color: #10b981; font-weight: 600;">👉 Click "Create" button</p></div>'
      }
    }
  },
  ...
  payment: {
    ...
    customMultiplierConflict: 'You already have an active {current} custom subscription. It cannot be changed to {selected} before it expires. Renew at the current multiplier, or buy again after expiration.',
    ...
    cafeCoupon: {
      ...
      appliedDiscount: 'Coupon applied successfully, {value}% off',
      appliedCash: 'Coupon applied successfully, redeem {amount}',
      invalid: 'Invalid Café coupon code',
      discount: 'Discount',
      discountLabel: 'Café coupon discount',
      payable: 'Payable',
      errors: {
    ...
    errors: {
      ...
      ACTIVITY_BENEFIT_CHANGED: 'The subscription bonus changed. Refresh the page and retry.',
      PROMOTION_ACTIVITY_NAME_REQUIRED: 'Enter an activity name.',
      PROMOTION_ACTIVITY_NAME_TOO_LONG: 'Activity name cannot exceed 100 characters.',
      PROMOTION_ACTIVITY_CURRENCY_INVALID: 'Gift currency must be CNY or USD.',
      PROMOTION_ACTIVITY_TYPE_INVALID: 'Unsupported activity type.',
      PROMOTION_ACTIVITY_WINDOW_INVALID: 'Activity end time must be later than its start time.',
      PROMOTION_ACTIVITY_MAX_USES_INVALID: 'Uses per user must be between 1 and 1000.',
      PROMOTION_ACTIVITY_PLANS_REQUIRED: 'Select at least one subscription plan.',
      PROMOTION_ACTIVITY_PLAN_INVALID: 'The plan or gift configuration is invalid.',
      PROMOTION_ACTIVITY_PRESALE_REQUIRED: 'Balance gifts require presale-enabled plans.',
      PROMOTION_ACTIVITY_PLAN_DUPLICATE: 'A subscription plan cannot be configured twice.',
      PROMOTION_ACTIVITY_PLAN_NOT_FOUND: 'One or more subscription plans no longer exist.',
      PROMOTION_ACTIVITY_VALIDITY_TOO_LONG: 'Plan validity plus bonus days exceeds the system limit.',
      PROMOTION_ACTIVITY_OVERLAP: 'An enabled activity already overlaps this time range for one or more selected plans.',
      PROMOTION_ACTIVITY_IMMUTABLE: 'An activity with participating orders can only change its name or enabled state.',
      PROMOTION_ACTIVITY_IN_USE: 'An activity with participating orders cannot be deleted.',
      SUBSCRIPTION_BONUS_REACQUIRE_FAILED: 'This order\'s bonus eligibility is now occupied by another order. Contact an administrator.',
      SUBSCRIPTION_BONUS_RESERVATION_MISSING: 'This order\'s bonus reservation record is missing. Contact an administrator.',
      PAYMENT_AFTER_EXPIRY_REFUND_REQUIRED: 'This payment arrived after the order grace period and can only be refunded.',
      PLAN_VALIDITY_TOO_LONG: 'The subscription plan validity exceeds the system limit.',
      PLAN_VALIDITY_UNIT_INVALID: 'The plan validity unit must be days, weeks, or months.',
      PLAN_ACTIVITY_VALIDITY_TOO_LONG: 'Plan validity plus an enabled activity bonus exceeds the system limit.',
    ...
    admin: {
      ...
      activityConfigHint: 'Configure time-bound plan activities with per-user usage limits.',
      activityRecords: 'Activity Records',
      activityRecordsHint: 'Review activity history, participants, and order-level participation results.',
      activityRecordSearch: 'Search activity, user, or order',
      activityPeriod: 'Activity Period',
      participantCount: 'Participants',
      participationCount: 'Participations',
      participationBreakdown: 'Reserved / Granted / Released',
      grantedBonusDays: 'Granted Bonus Days',
      viewParticipants: 'View Participants',
      participant: 'Participant',
      participantsHint: 'Participants are unique users; counts retain every order record.',
      participationRecordsHint: 'Order-level participation records for this user and activity.',
      noActivityRecords: 'No activity records',
      noParticipants: 'No participants',
      noParticipationRecords: 'No order participation records',
      firstParticipatedAt: 'First Participated',
      lastParticipatedAt: 'Last Participated',
      ...
      deleteActivityConfirm: 'Delete this activity? Activities with participation records cannot be deleted.',
      noActivities: 'No activities configured',
      activityName: 'Activity Name',
      activityType: 'Activity Type',
      subscriptionBonusDays: 'Subscription Bonus Days',
      activityStartsAt: 'Starts At',
      activityEndsAt: 'Ends At',
      maxUsesPerUser: 'Uses Per User',
      maxUsesPerUserHint: 'The limit is shared across plans in this activity. With 1 use, any participating plan consumes the benefit.',
      activityPlans: 'Plans and Bonus Days',
      activityEnabled: 'Enable Activity',
      activityPlansRequired: 'Select at least one participating plan',
      activityBonusDaysRequired: 'Bonus days must be greater than zero for every selected plan',
      activityPlanCount: '{count} plans',
      activityUserLimit: '{count} per user',
      activityStatus: {
        disabled: 'Disabled',
        scheduled: 'Scheduled',
        active: 'Active',
      ...
      customMultiplierEnabled: 'Allow custom multiplier',
      customMultiplierMin: 'Min multiplier',
      ...
      earlyResetEnabled: 'One-time quota with early reset',
      earlyResetHint: 'Makes quota one-time across accumulated subscription validity, with no daily, weekly, or monthly auto-reset. Users may reset it early in exchange for fewer remaining days.',
      earlyResetDurationDays: 'Days deducted per reset',
      earlyResetDurationHint: 'Number of days deducted from the subscription\'s accumulated remaining time after each early reset.',

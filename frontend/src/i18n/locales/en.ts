  ...
  admin: {
    ...
    riskControl: {
      ...
      requestControl: {
        ...
        protocolBlockingHint: 'When off, the policy still evaluates and records the expected result, but the request is observed and allowed without blocking, counting, or notifications. Anonymous Responses requests remain blocked, except explicit or strong-shape compaction requests, which are observed and allowed.',
        ...
        logsHint: 'Records are aggregated by user and request-format fingerprint with count and first/last seen time. Details keep the latest request context for that fingerprint, multi-value headers with authentication credentials redacted, a head/tail body snapshot, full-body hash, session source, and compaction evidence. Body snapshots are capped at 256KB and kept for 3 days; base records are kept for 30 days.',
        ...
        snapshotUnavailable: 'No request snapshot is available. The record may predate diagnostic snapshots or its snapshot may have exceeded the 3-day retention period.',
        requestBodyMetadata: 'Request body metadata',
        detailBodyHint: 'Structured metadata supports quick inspection; use the request body snapshot above to verify the actual content. Explicit or strong-shape compaction requests are observed and allowed.',

  ...
  admin: {
    ...
    riskControl: {
      ...
      requestControl: {
        ...
        requestSnapshot: '同时保存观察请求诊断快照',
        requestSnapshotHint: '实际拦截请求始终强制保存诊断快照。开关默认关闭；开启后也为观察请求保存脱敏请求头和最多 256KB 的正文头尾，快照保留 3 天；关闭仅停止观察请求的新快照。',
        ...
        noUserRules: '暂无用户规则',
        logsTitle: '管控记录',
        logsHint: '按用户与请求格式指纹聚合，展示次数和首次/末次时间。实际拦截请求强制保存诊断快照；开启快照开关后也保存观察请求。详情最多每 15 分钟刷新一次，正文快照最多 256KB、保留 3 天；基础记录保留 30 天。',
        refresh: '刷新记录',
        ...
        snapshotUnavailable: '该记录没有请求快照：可能是历史记录、快照已超过 3 天保留期，或采集/写入过程发生降级。',

...
export default {
  ...
  membership: {
    ...
    cafeCoupon: {
      ...
      claimed: '已领取',
      view: '查看',
      alreadyClaimed: '本期 Café券已领取',
      disabled: '未开启',
      tip: 'Café券可在充值或订阅付款时享受折扣，具体权益与当前会员等级相关。',
      remainingDays: '{days} 天后可再次领取',
      nextClaimLater: '下个周期可再次领取',
      claimDiscountHint: '每{period}可获得一张，当前等级折扣为{value}%',
      claimCashHint: '每{period}可获得一张，当前等级可抵扣{value}元',
      benefitDiscount: '折扣 {value}%',
      benefitCash: '抵扣 ¥{value}',
      period: {
        day: '天',
        week: '周',
        month: '30天'
      ...
      modalTitle: 'Café Coupon',
      modalDescription: '充值或订阅确认付款时可使用此券码。',
      nonTransferableHint: '本账号使用',
      codeLabel: '券码',
      copy: '复制',
      use: '使用',
      copied: '券码已复制',
  ...
  keys: {
    ...
    endpoints: {
      ...
      latency: '延迟',
      checkingLatency: '正在检测延迟',
      latencyUnavailable: '延迟检测失败',
    },
    allGroups: '全部分组',
    allStatus: '全部状态',
    createKey: '创建密钥',
    ...
    keyRotatedSuccess: 'API 密钥已轮转，请复制新的密钥',
    rotate: '轮转',
    rotateKey: '轮转密钥',
    rotateConfirmMessage: '确定要轮转「{name}」吗？旧密钥会立即失效，新的完整密钥只会显示一次。',
    failedToRotate: '轮转 API 密钥失败',
    keyOnlyShownOnceShort: '完整密钥只会在创建或轮转时显示一次',
    oneTimeKey: {
      title: '复制新的 API 密钥',
      description: '请现在复制并保存此 API 密钥。关闭此窗口后，后台将无法再次显示完整密钥。',
      warning: '此密钥只会在此出现一次。',
      copy: '复制密钥',
      closeConfirmTitle: '关闭前确认',
      closeConfirmMessage: '关闭后将无法再次查看完整密钥。确定要关闭吗？',
      closeConfirmConfirm: '确认关闭'
    },
  ...
  availableChannels: {
    catalog: {
      ...
      modelPrice: '模型价格',
      listPrice: '原价',
      multiplierHint: '模型价格 = 原价 × 此倍率',
      priceDetails: '查看详情',
      perRequestUnit: 'USD / 次',
      groups: '可用分组',
      groupMultiplier: '分组倍率（含已配置的专属倍率）',
      noResults: '没有匹配的模型，试试其他关键词。',
    },
    title: '可用渠道',
  ...
  profile: {
    ...
    concurrencyRules: {
      title: '并发规则',
      subscription: '有生效订阅时，按订阅并发计算；多份订阅取最高值，不累加。',
      balance: '无生效订阅时，按当前账户余额计算：',
      interval: '{min} ≤ 余额 < {max}',
      unbounded: '余额 ≥ {min}（无上限）',
      note: '待生效预售不计入。所有 API Key 共用此上限，随余额和订阅状态动态调整。',
    },
    rpmLimit: 'RPM 限制',
    rpmUnlimited: '不限制',
    memberSince: '注册时间',
  ...
  admin: {
    ...
    users: {
      ...
      concurrencyRules: {
        title: '并发规则',
        description: '全局规则适用于无生效订阅的用户，按当前账户余额（$）计算，不是人民币充值金额。有生效订阅时仍优先使用订阅并发；多份订阅取最高值，不累加。保存后的规则用于新请求，不取消进行中的请求。',
        coverage: '各档位包含下限、不包含上限，上限自动取下一档下限。从 $0 开始严格递增，确保区间连续、无重叠或遗漏；最高档无上限。',
        tier: '档位 {number}',
        minBalance: '余额下限（$，包含）',
        upperBalance: '余额上限（$，不包含）',
        concurrency: '并发请求数',
        firstFixed: '首档下限固定为 $0。',
        noUpperBound: '无上限',
        pendingBound: '请填写有效的下一档下限',
        addTier: '添加档位',
        removeTier: '删除档位',
        removeTierLabel: '删除档位 {number}',
        tierCount: '{count} / 20 个档位（至少 1 个）',
        retry: '重新加载',
        loadFailed: '加载并发规则失败。加载成功前无法保存。',
        saveFailed: '保存并发规则失败，已保留当前编辑内容，请重试。',
        saved: '并发规则已保存',
        validation: {
          summary: '请设置 1–20 个连续档位，并修正标记的数值后保存。',
          balance: '请输入 $0 至 $1,000,000,000,000 之间的有限余额数值。',
          order: '首档从 $0 开始，后续每档下限必须严格大于前一档。',
          concurrency: '并发请求数必须为 1–1000 的整数。'
        }
      },
      ...
      recordUserBalanceHistory: '写入用户充值记录',
      auditOnlyAdjustment: '关闭时仅写入后台审计日志',
      ...
      currentMembershipLevel: '当前会员等级',
      membershipPoints: '会员点数',
      membershipPointsUnit: '点',
      membershipPointsTip: '点击修改会员点数',
      newMembershipPoints: '操作后会员点数',
      newMembershipLevel: '操作后会员等级',
      setPoints: '设置点数',
      addPoints: '增加点数',
      subtractPoints: '减少点数',
      pointsRequired: '请输入有效点数',
      insufficientMembershipPoints: '会员点数不足',
    ...
    groups: {
      ...
      customSubscription: {
        ...
        sourceGroup: '来源分组',
        multiplier: '当前定制倍数',
        multiplierHint: '保存后后端会按来源分组重算日/周/月限额，不会修改历史订单金额。',
        multiplierInvalid: '定制倍数必须大于等于 1。',
      },
      subscription: {
        title: '订阅设置',
        type: '计费类型',
    ...
    channels: {
      ...
      form: {
        ...
        billingMode: '计费模式',
        listPrices: '原价与倍率',
        priceMultiplier: '模型倍率',
        priceMultiplierHint: '以下均为原价，包含缓存和阶梯价格。模型价格 = 原价 × 模型倍率，分组或专属倍率另计。留空价格沿用现有回退规则。',
        invalidPriceMultiplier: '模型倍率必须大于 0 且不超过 1000',
        defaultPrices: '默认原价（未命中区间时使用）',
        inputPrice: '输入',
        outputPrice: '输出',
        cacheWritePrice: '缓存写入',
        cacheReadPrice: '缓存读取',
        ...
        defaultPerRequestPrice: '默认单次原价（未命中层级时使用）',
        defaultImagePrice: '默认图片原价（未命中层级时使用）',
        platformConfig: '平台配置',
    ...
    riskControl: {
      title: '风控中心',
      description: '配置内容审计与请求管控策略，并查看对应记录',
      loadFailed: '加载风控中心失败',
      ...
      contentHitPolicy: '内容审计命中策略',
      contentHitPolicyHint: '仅统计内容审计与关键词命中，不包含 Cyber 上游拦截。',
      emailOnHit: '命中后发送邮件',
      emailOnHitHint: '开启后每次达到阈值都会向用户发送风控提醒邮件；自动封禁通知始终发送。',
      autoBan: '自动封禁用户',
      autoBanHint: '命中次数达到阈值后将禁用用户账号、刷新认证缓存并发送封禁通知邮件。',
      cyberPolicyEnabled: '启用 Cyber 上游风控处理',
      cyberPolicyEnabledHint: '识别上游 400 Cyber 或 cyber_policy 响应，并在风控中心记录对应用户。',
      cyberPolicySection: 'Cyber 上游风控',
      cyberPolicySectionHint: 'Cyber 命中使用独立的累计次数、封禁阈值和时间窗口，不与内容审计共用。',
      cyberPolicyEmail: '发送 Cyber 滥用提醒邮件',
      cyberPolicyEmailHint: '命中后通过邮件系统通知对应 CafeCode 用户。',
      cyberPolicyEmailMessage: 'Cyber 邮件正文提示语',
      cyberPolicyEmailMessageHint: '只替换邮件开头的说明句；留空时按收件人语言使用默认文案。',
      cyberPolicyEmailMessagePlaceholder: '您的请求被上游服务商的网络安全策略（cyber policy）拦截。',
      cyberPolicyAutoBan: 'Cyber 命中自动封禁',
      cyberPolicyAutoBanHint: '独立 Cyber 次数达到阈值后禁用用户并发送封禁通知。',
      cyberPolicyBanThreshold: 'Cyber 封禁触发次数',
      cyberPolicyViolationWindowHours: 'Cyber 累计窗口（小时）',
      banThreshold: '封禁触发次数',
      ...
      requestControl: {
        ...
        enabled: '启用请求管控',
        protocolBlocking: '协议拦截开关',
        protocolBlockingHint: '关闭后仍按原规则判断并记录预期结果，但请求只观察放行，不执行拦截、累计和通知副作用。匿名 Responses 请求仍强制拦截；明确压缩信号只观察放行，结构强特征仅补充会话后继续正常策略。',
        blockOpenAIChat: '拦截 OpenAI Chat 请求',
        blockClaudeMessages: '拦截 Claude Messages 请求',
        blockOpenAIResponses: '拦截 OpenAI Responses 请求',
        requestSnapshot: '保存请求诊断快照',
        requestSnapshotHint: '开关开启时，为被拦截或观察的请求保存脱敏请求头和最多 256KB 的正文头尾，快照保留 3 天；关闭后不采集任何新快照。',
        requestSnapshotRuntime: '服务运行时快照开关',
        globalSwitchOff: '系统设置中的“风控中心”总开关当前关闭，请求管控不会实际执行。',
        save: '保存管控配置',
        saved: '请求管控配置已保存',
        loadFailed: '加载请求管控配置失败',
        statusFailed: '加载请求管控状态失败',
        logsFailed: '加载请求管控记录失败',
        scopeTitle: '管控范围',
        scopeHint: '请求热路径使用内存快照；修改配置后自动刷新。',
        groups: '分组范围',
        allGroups: '全部分组',
        selectedGroups: '仅指定分组',
        noGroups: '暂无可用分组',
        models: '模型范围',
        modelsPlaceholder: '每行一个模型名',
        globalUA: '全局 User-Agent 白名单',
        uaPlaceholder: '每行一个 UA 前缀，命中后跳过请求管控',
        uaHint: '用户级白名单会与全局列表合并，不会替换全局配置。',
        blockStatus: '拦截状态码',
        blockStatusHint: '客户端将使用此 HTTP 状态码，默认 403。',
        blockMessage: '拦截提示',
        blockMessageHint: '客户端拦截响应和请求管控通知邮件都会使用此提示。',
        notificationsTitle: '命中通知与自动封禁',
        notificationsHint: '命中记录在后台异步计数；同一用户两次命中间隔超过 5 分钟才累计一次。',
        emailOnHit: '命中后发送邮件',
        emailOnHitHint: '开启后每次计入累计次数都会发送风控提醒邮件；自动封禁通知始终发送。',
        autoBan: '自动封禁用户',
        autoBanHint: '达到阈值后禁用用户账号、刷新认证缓存并发送封禁通知邮件。',
        banThreshold: '封禁触发次数',
        violationWindowHours: '累计窗口（小时）',
        violationWindowHint: '只有距离该用户上次命中请求超过 5 分钟，才会新增一次累计次数。',
        violationCount: '累计 {count} 次',
        notCounted: '本次未新增累计',
        emailSent: '已发邮件',
        hitEmailSent: '命中提醒已发',
        banEmailSent: '封禁通知已发',
        autoBanned: '已封禁',
        usersTitle: '用户范围与例外',
        usersHint: '默认全体用户参与；关闭后仅参与列表中的用户进入管控。',
        searchUser: '按邮箱、用户名或 UID 搜索用户',
        searching: '搜索中',
        noUsersFound: '未找到用户',
        selectedUser: '已选择：{user}',
        userSearchFailed: '搜索用户失败',
        userID: '用户 ID',
        userUAPlaceholder: '用户 UA 前缀，使用逗号或换行分隔',
        addUser: '添加用户',
        updateUser: '更新用户',
        editUserUA: '编辑用户 UA 白名单',
        allUsersOn: '未配置例外时全体用户参与',
        onlyConfiguredUsers: '仅配置为参与的用户进入管控',
        ruleCount: '{count} 条用户规则',
        participate: '参与',
        userUA: '用户 UA 白名单',
        removeUser: '移除用户规则',
        noUserRules: '暂无用户规则',
        logsTitle: '管控记录',
        logsHint: '按用户与请求格式指纹聚合，展示次数和首次/末次时间。快照开关开启时保存被拦截及观察请求，关闭时不采集新快照。详情最多每 15 分钟刷新一次，正文快照最多 256KB、保留 3 天；基础记录保留 30 天。',
        refresh: '刷新记录',
        search: '搜索用户、UA、原因或请求 ID',
        applyFilters: '应用筛选',
        allActions: '全部结果',
        blocked: '已拦截',
        observed: '后台观察',
        ...
        requestID: '请求 ID',
        eventCount: '聚合 {count} 次',
        singleEvent: '命中 1 次',
        eventCountLabel: '聚合事件数',
        firstSeen: '首次出现',
        lastSeen: '最后出现',
        requestHeaders: '请求头（已脱敏）',
        requestHeadersSummary: '请求头摘要（兼容历史记录）',
        fullRequestHeaders: '完整请求头（认证凭据已脱敏）',
        requestBodySnapshot: '请求正文诊断快照',
        requestMethod: '请求方法',
        requestHost: '请求主机',
        requestPath: '请求路径与查询参数',
        clientAddress: '客户端地址',
        bodySize: '原始正文大小',
        capturedSize: '已保存正文大小',
        snapshotCapturedAt: '快照采集时间',
        bodyTruncatedHint: '正文超过快照上限，只保存头尾 {captured}，原始共 {total}；可用 SHA-256 校验原始内容。',
        snapshotUnavailable: '该记录没有请求快照：可能当时快照开关未开启、属于历史记录、快照已超过 3 天保留期，或采集/写入过程发生降级。',
        snapshotUnavailableReason: '采集状态：{reason}',
        requestBodyMetadata: '请求体元数据',
      ...
      keywordWhitelist: '关键词白名单',
      keywordWhitelistPlaceholder: '每行输入一个白名单关键词，例如：\n正常业务词\n专有名词',
      keywordWhitelistCount: '已配置 {count} 个白名单词',
      keywordWhitelistHint: '白名单优先级高于内置过滤库和拦截关键词；命中白名单后只跳过关键词拦截，不影响 API 审计策略。',
      ...
      builtInFilterEnabled: '启用内置过滤库',
      builtInFilterEnabledDesc: '启用后，内置过滤库与本页配置的拦截关键词同时生效；关闭时仅使用本页配置。',
      builtInFilterCategories: '内置词库分类',
      builtInFilterCategoriesDesc: '只匹配已勾选分类的内置词；默认全分类生效。',
      builtInFilterLevels: '内置词库等级',
      builtInFilterLevelsDesc: '只匹配已勾选等级的内置词；默认仅 high 生效。',
      builtInFilterSelectedCount: '已选 {count} 项',
      builtInFilterCategoryPolitical: '政治',
      builtInFilterCategoryPornographic: '色情',
      builtInFilterCategoryViolence: '暴力',
      builtInFilterCategoryAbuse: '辱骂',
      builtInFilterCategoryAd: '广告导流',
      builtInFilterCategoryIllegal: '违法',
      builtInFilterCategoryOther: '其他',
      builtInFilterLevelLow: '低危',
      builtInFilterLevelMedium: '中危',
      builtInFilterLevelHigh: '高危',
      builtInFilterLevelCritical: '严重',
      overview: {
        status: '运行状态',
        enabled: '已启用',
        disabled: '未启用',
    ...
    subscriptions: {
      ...
      resetQuota: '重置配额',
      resetCount: '可重置次数',
      setResetCount: '设置重置次数',
      setResetCountTitle: '设置订阅重置次数',
      resetCountLabel: '剩余重置次数',
      resetCountHint: '设置用户可自助重置每日/每周额度的次数（0-{max}）。',
      settingResetCount: '保存中...',
      resetCountUpdated: '订阅重置次数已更新',
      failedToSetResetCount: '设置订阅重置次数失败',
      bulkSetResetCount: '批量设置重置次数',
      bulkSetResetCountSelected: '设置已选 {count} 个',
      bulkSetResetCountSelectedMessage: '将为已选的 {count} 个订阅设置相同的剩余重置次数；不支持日/周额度的订阅在设置大于 0 时会跳过（设置为 0 可清理旧次数）。',
      bulkSetResetCountTitle: '批量设置订阅重置次数',
      bulkSetResetCountMessage: '将为所有有效订阅设置相同的剩余重置次数；不支持日/周额度的订阅在设置大于 0 时会跳过（设置为 0 可清理旧次数）。',
      bulkResetCountSuccess: '已更新 {updated} 个订阅，跳过 {skipped} 个',
      selectAllForResetCount: '全选订阅以设置重置次数',
      selectForResetCount: '选择订阅以设置重置次数',
      resetCountErrors: {
        INVALID_QUOTA_RESET_COUNT: '重置次数必须是 0 到 1000 之间的整数',
        QUOTA_RESET_DISABLED: '该订阅不支持日/周额度的自助重置次数设置',
        QUOTA_RESET_UNAVAILABLE: '重置次数功能暂不可用，请稍后再试',
        QUOTA_RESET_CONFLICT: '订阅状态已变化，请刷新后重试',
        TOO_MANY_SUBSCRIPTIONS: '单次最多设置 1000 个订阅',
      ...
      bulkResetQuota: '重置全部配额',
      bulkResetQuotaTitle: '重置全部订阅配额',
      bulkResetQuotaConfirm: '请选择要重置的用量窗口。此操作会影响全部有效订阅，不受当前筛选或分页限制，所选窗口的用量将归零并从今天开始重新计算。',
      bulkResetQuotaDaily: '每日用量',
      bulkResetQuotaWeekly: '每周用量',
      bulkResetQuotaMonthly: '每月用量（会影响月额度订阅，请谨慎勾选）',
      bulkResetQuotaNoWindow: '请至少选择一个用量窗口。',
      bulkQuotaResetSuccess: '已重置 {count} 个订阅的配额',
      failedToBulkResetQuota: '重置全部配额失败',
      shiftWindow: '延长重置窗口',
      shiftWindowTitle: '统一延长重置窗口',
      shiftWindowWarning: '此操作会批量修改订阅的窗口起点，只改变下次重置时间，不会改变任何已用量。请确认筛选范围后再提交。',
      shiftWindowWindows: '要移动的窗口',
      shiftWindowDaily: '日窗口',
      shiftWindowWeekly: '周窗口',
      shiftWindowMonthly: '月窗口',
      shiftWindowNoWindow: '请至少选择一个窗口。',
      shiftWindowOffsetHours: '偏移小时数',
      shiftWindowOffsetHint: '正数推迟重置、负数提前；不能为 0，绝对值不超过 {max} 小时。',
      shiftWindowOffsetRequired: '请输入非 0 的偏移小时数。',
      shiftWindowOffsetInteger: '偏移小时数必须是整数。',
      shiftWindowOffsetRange: '偏移小时数的绝对值不能超过 {max}。',
      shiftWindowScope: '作用范围',
      shiftWindowScopeHint: '作用范围跟随页面当前筛选，不受分页限制。',
      shiftWindowScopePlatform: '平台',
      shiftWindowPreview: '预计影响 {matched} 个订阅，其中 {skipped} 个因窗口起点会落到未来被跳过。',
      shiftWindowPreviewLoading: '正在计算影响范围…',
      shiftWindowPreviewUnavailable: '填写有效的窗口和偏移小时数后显示预计影响范围。',
      shiftWindowSubmit: '确认延长',
      shiftWindowSubmitting: '提交中...',
      shiftWindowSuccess: '已调整 {count} 个订阅的重置窗口，跳过 {skipped} 个',
      shiftWindowFailed: '调整重置窗口失败',
      stats: {
        openButton: '订阅统计',
        title: '订阅统计',
        detailTitle: '订阅使用率明细',
        remainingToday: '今日剩余额度',
        remainingWeek: '本周剩余额度',
        horizonCapacity: '未来可消耗上限',
        horizonOption: '{days} 天',
        dailyLimitedCount: '{count} 个日限订阅',
        weeklyLimitedCount: '{count} 个周限订阅',
        totalSubscriptions: '全部 {count} 个订阅',
        byPlan: '分套餐明细',
        colPlan: '套餐',
        colSubscriptions: '订阅数',
        colRemainingToday: '今日剩余',
        colRemainingWeek: '本周剩余',
        colRemainingMonth: '本月剩余',
        colHorizon: '{days} 天上限',
        pendingReset: '待重置',
        colUsed: '已用',
        colUsageRatio: '使用率',
        ...
        weeklyUsage: '每周使用率',
        derived: '折算值',
        derivedDailyHint: '该套餐没有日限额，分母按 周限 ÷ 7 折算',
        derivedWeeklyHint: '该套餐没有周限额，分母按 日限 × 7 折算',
        historyTruncated: '历史数据自 {date} 起，更早的用量已随 usage_logs 保留期清理，无法回溯。',
        historyEmpty: '暂无历史用量数据，汇总表刚刚建立，需要一段时间才能积累完整周期。',
        generatedAt: '统计时间：{time}',
        noData: '暂无数据',
        loadFailed: '加载订阅统计失败',
        seriesLoadFailed: '加载使用率明细失败'
      },
      ...
      guide: {
        ...
        actions: {
          ...
          resetCountDesc: '设置用户可自助重置日/周额度的剩余次数',
          resetQuota: '重置配额',
    ...
    promo: {
      ...
      cafeCoupon: {
        ...
        changeStatus: '修改状态',
        targetStatus: '目标状态',
        statusUpdated: 'Café券状态已更新',
        statusNotAllowed: '已完成支付订单的 Café券不能恢复为未使用',
        failedToUpdateStatus: '更新 Café券状态失败',
        resetClaimPeriod: '重置领取周期',
        resetClaimPeriodConfirm: '确定要重置该用户的 Café券领取资格吗？系统会调整该用户最新一张 Café券的领取周期；如果最新券为已发放且未使用，也会被作废。用户可立即重新领取。',
        claimPeriodReset: '领取周期已重置',
        failedToResetClaimPeriod: '重置领取周期失败',
        voidCoupon: '作废 Café券',
        voidCouponConfirm: '确定要作废这张 Café券吗？此操作无法撤销。',
        voided: 'Café券已作废',
        voidNotAllowed: '只能作废已发放且未使用的 Café券',
        failedToLoad: '加载 Café券失败',
        failedToVoid: '作废 Café券失败'
      }
    },
    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          membershipThresholds: '会员等级点数阈值',
          membershipThresholdsDesc: 'LV.1 > 20，LV.2 > 300，LV.3 > 1000，LV.4 > 3000，LV.5 > 5000。',
          rebateRate: '会员等级返利比例',
          rebateRateHint: '按邀请人的会员等级设置默认金额返利比例；LV.0 无邀请和返利资格，专属用户配置优先。默认：LV.1 5%，LV.2 5%，LV.3 8%，LV.4 10%，LV.5 12%。',
          ...
          perInviteeCap: '返利上限',
          perInviteeCapDesc: '按邀请人的会员等级设置每个被邀请用户最多产生的返利总额。0 表示不能获得返利。默认：普通用户 0，LV.1 100，LV.2 300，LV.3-LV.5 1000。',
          ...
          inviteLimitDesc: '按邀请人的会员等级设置最大邀请人数。0 = 暂不可邀请；可在下方为指定用户单独覆盖。默认：普通用户 0，LV.1 1，LV.2 3，LV.3-LV.5 5。',
        ...
        cafeCoupon: {
          title: 'Café 券',
          description: '按会员等级配置 Café 券权益。默认：LV.1 5 元代金券、LV.2 10 元代金券、LV.3 10% 折扣、LV.4-LV.5 15% 折扣；仅 LV.5 默认可给他人使用。券有效期按领取周期的 30% 计算。',
          level0: '普通用户 / LV.0',
          level1: 'LV.1',
          level2: 'LV.2',
          level3: 'LV.3',
          ...
          monthEndValidityTitle: '有效期为领取周期的 30%',
          monthEndValidityHint: '每天领取约 7.2 小时，每周领取约 2.1 天，每 30 天领取为 9 天。',
          valueHint: '现金券按金额填写；折扣券填写 0-100 的折扣百分比。面值为 0 时后端会关闭该等级。',
        },
      },
      ...
      payment: {
        ...
        guestShopEnabledHint: '该开关与上方原站支付开关完全独立；关闭后只禁止新游客发起支付。',
        guestShopStripeInstance: '游客支付 Stripe 实例',
        guestShopStripeInstanceHint: '可选择原站已禁用的 Stripe 实例。游客支付只读该实例的密钥、币种和支持方式，不修改实例状态。',
        guestShopStripeNotSelected: '请选择 Stripe 实例',
        guestShopStripeOption: '{name}（ID {id}）· {status}',
        guestShopNoStripeInstance: '当前没有可选的 Stripe 实例，请先在服务商管理中创建并完整配置一个 Stripe 实例。',
        guestShopSelectionRequired: '启用游客支付前必须选择一个配置完整的 Stripe 实例。',
        guestShopOriginalStatus: '原站实例状态',
        guestShopOriginalEnabled: '原站已启用',
        guestShopOriginalDisabled: '原站已禁用',
        guestShopCurrency: '币种',
        guestShopMethods: '支持方式',
        guestShopNotConfigured: '未配置',
  ...
  userSubscriptions: {
    ...
    resetQuotaResetting: '重置中...',
    resetQuotaSuccess: '订阅每日/每周额度已重置',
    resetQuotaFailed: '重置订阅额度失败',
    resetQuotaErrors: {
      INVALID_QUOTA_RESET_COUNT: '重置次数必须是 0 到 1000 之间的整数',
      QUOTA_RESET_DISABLED: '该订阅不支持自助重置额度',
      QUOTA_RESET_EXHAUSTED: '该订阅没有剩余重置次数',
      QUOTA_RESET_CONFLICT: '订阅状态已发生变化，请刷新后重试',
      QUOTA_RESET_UNAVAILABLE: '自助重置暂不可用，请稍后再试',
      SUBSCRIPTION_EXPIRED: '该订阅已过期',
      SUBSCRIPTION_SUSPENDED: '该订阅已暂停',
      SUBSCRIPTION_NOT_STARTED: '该订阅尚未开始',
      SUBSCRIPTION_NOT_FOUND: '订阅不存在'
    },
    earlyResetErrors: {
  ...
  onboarding: {
    ...
    admin: {
      ...
      keySubmit: {
        title: '🎉 生成并复制',
        description:
          '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;">点击创建后，系统会生成完整的 API Key。</p><div style="padding: 8px 12px; background: #fee2e2; border-left: 3px solid #ef4444; border-radius: 4px; font-size: 13px; margin-bottom: 12px;"><b>⚠️ 重要提醒：</b><ul style="margin: 8px 0 0 16px;"><li>密钥只显示一次，请立即复制</li><li>丢失后需要重新生成</li><li>妥善保管，不要泄露给他人</li></ul></div><div style="padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #10b981; border-radius: 4px; font-size: 13px; margin-bottom: 12px;"><b>🚀 下一步：</b><ul style="margin: 8px 0 0 16px;"><li>复制生成的 cafepass-uid-xxxxx 密钥</li><li>在支持 OpenAI 接口的客户端中使用</li><li>开始体验 AI 服务！</li></ul></div><p style="margin-top: 12px; color: #10b981; font-weight: 600;">👉 点击"创建"按钮</p></div>'
      }
    },
    // User tour steps
    user: {
      ...
      keySubmit: {
        title: '🎉 完成创建',
        description:
          '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;">点击确认创建您的 API 密钥。</p><div style="padding: 8px 12px; background: #fee2e2; border-left: 3px solid #ef4444; border-radius: 4px; font-size: 13px; margin-bottom: 12px;"><b>⚠️ 重要：</b><ul style="margin: 8px 0 0 16px;"><li>创建后请立即复制密钥（cafepass-uid-xxxxx）</li><li>密钥只显示一次，丢失需重新生成</li></ul></div><p style="padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #10b981; border-radius: 4px; font-size: 13px;"><b>🚀 如何使用：</b><br/>将密钥配置到支持 OpenAI 接口的任何客户端（如 ChatBox、OpenCat 等），即可开始使用！</p><p style="margin-top: 12px; color: #10b981; font-weight: 600;">👉 点击"创建"按钮</p></div>'
      }
    }
  },
  ...
  payment: {
    ...
    cafeCoupon: {
      ...
      applied: 'Café券已应用',
      appliedDiscount: '券成功应用，折扣 {value}%',
      appliedCash: '券成功应用，抵扣 {amount}',
      invalid: '券码无效',
      discount: '优惠',
      discountLabel: 'Café券优惠',
      payable: '应付',
      errors: {
    ...
    errors: {
      ...
      DAILY_LIMIT_EXCEEDED: '本次付款将超出每日购买限额，今日剩余额度 {remaining}。',
      ...
      ACTIVITY_BENEFIT_CHANGED: '套餐赠送活动已变更，请刷新页面后重试',
      PROMOTION_ACTIVITY_NAME_REQUIRED: '请输入活动名称',
      PROMOTION_ACTIVITY_NAME_TOO_LONG: '活动名称不能超过 100 个字符',
      PROMOTION_ACTIVITY_CURRENCY_INVALID: '赠金币种必须为人民币或美元',
      PROMOTION_ACTIVITY_TYPE_INVALID: '不支持该活动类型',
      PROMOTION_ACTIVITY_WINDOW_INVALID: '活动结束时间必须晚于开始时间',
      PROMOTION_ACTIVITY_MAX_USES_INVALID: '每用户生效次数必须在 1 到 1000 之间',
      PROMOTION_ACTIVITY_PLANS_REQUIRED: '请至少选择一个订阅套餐',
      PROMOTION_ACTIVITY_PLAN_INVALID: '套餐或赠送额度配置无效',
      PROMOTION_ACTIVITY_PRESALE_REQUIRED: '赠送余额活动仅支持已启用预售的套餐。',
      PROMOTION_ACTIVITY_PLAN_DUPLICATE: '同一套餐不能重复配置',
      PROMOTION_ACTIVITY_PLAN_NOT_FOUND: '一个或多个订阅套餐不存在',
      PROMOTION_ACTIVITY_VALIDITY_TOO_LONG: '套餐有效期与赠送天数之和超过系统上限',
      PROMOTION_ACTIVITY_OVERLAP: '所选套餐在该时间段内已有启用中的同类活动',
      PROMOTION_ACTIVITY_IMMUTABLE: '已有订单参与的活动只能修改名称或启用状态',
      PROMOTION_ACTIVITY_IN_USE: '已有订单参与的活动不能删除',
      SUBSCRIPTION_BONUS_REACQUIRE_FAILED: '该订单的赠送资格已被其他订单占用，请联系管理员处理',
      SUBSCRIPTION_BONUS_RESERVATION_MISSING: '该订单的赠送资格记录缺失，请联系管理员处理',
      PAYMENT_AFTER_EXPIRY_REFUND_REQUIRED: '该支付已超过订单有效期宽限时间，只能退款处理',
      PLAN_VALIDITY_TOO_LONG: '套餐有效期超过系统上限',
      PLAN_VALIDITY_UNIT_INVALID: '套餐有效期单位必须是天、周或月',
      PLAN_ACTIVITY_VALIDITY_TOO_LONG: '套餐有效期与启用活动赠送天数之和超过系统上限',
    ...
    admin: {
      ...
      activityConfig: '活动配置',
      activityConfigHint: '配置有起止时间和用户次数限制的套餐活动。',
      activityRecords: '活动记录',
      activityRecordsHint: '查看活动历史、参与用户以及每笔订单的参与结果。',
      activityRecordSearch: '搜索活动、用户或订单',
      activityPeriod: '活动时间',
      participantCount: '参与人数',
      participationCount: '参与次数',
      participationBreakdown: '预占 / 已赠送 / 已释放',
      grantedBonusDays: '累计赠送天数',
      viewParticipants: '查看参与者',
      participant: '参与者',
      participantsHint: '参与者按用户去重，次数统计保留全部订单记录。',
      participationRecordsHint: '展示该用户在活动中的订单级参与记录。',
      noActivityRecords: '暂无活动记录',
      noParticipants: '暂无参与者',
      noParticipationRecords: '暂无订单参与记录',
      firstParticipatedAt: '首次参与',
      lastParticipatedAt: '最近参与',
      backToActivities: '返回活动记录',
      backToParticipants: '返回参与者',
      participationStatusLabel: '参与状态',
      participationStatus: {
        reserved: '已预占',
        granted: '已赠送',
        released: '已释放',
      ...
      editActivity: '编辑活动',
      deleteActivity: '删除活动',
      deleteActivityConfirm: '确定要删除此活动吗？已有用户参与记录的活动不能删除。',
      noActivities: '暂无活动',
      activityName: '活动名称',
      activityType: '活动类型',
      subscriptionBonusDays: '订阅套餐赠送天数',
      activityStartsAt: '开始时间',
      activityEndsAt: '结束时间',
      maxUsesPerUser: '每用户生效次数',
      maxUsesPerUserHint: '次数在该活动内跨套餐共享；设为 1 时，购买任一参与套餐后不再重复赠送。',
      activityPlans: '参与套餐与赠送天数',
      activityEnabled: '启用活动',
      activityPlansRequired: '请至少选择一个参与套餐',
      activityBonusDaysRequired: '所选套餐的赠送天数必须大于 0',
      activityPlanCount: '{count} 个套餐',
      activityUserLimit: '每用户 {count} 次',
      activityStatus: {
        disabled: '已停用',
        scheduled: '未开始',
        active: '进行中',
      ...
      rateMultiplier: '费率倍数',
      customMultiplier: '定制倍数',
      customMultiplierEnabled: '允许定制倍数',
      customMultiplierMin: '最小倍数',
      ...
      earlyResetEnabled: '限时额度（允许提前重置）',
      earlyResetHint: '开启后，该套餐为限时额度：不会按日、周或月自动重置；用户可主动重置全部额度，代价是缩短订阅时间。',
      earlyResetDurationDays: '每次缩短天数',
      earlyResetDurationHint: '每次提前重置后，从订阅累计剩余时间中扣除的天数。',

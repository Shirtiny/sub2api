  ...
  admin: {
    ...
    riskControl: {
      ...
      requestControl: {
        ...
        protocolBlockingHint: '关闭后仍按原规则判断并记录预期结果，但请求只观察放行，不执行拦截、累计和通知副作用。匿名 Responses 请求仍强制拦截；明确或强特征命中的压缩请求例外，只观察放行。',
        ...
        noUserRules: '暂无用户规则',
        logsTitle: '管控记录',
        logsHint: '按用户与请求格式指纹聚合，展示次数和首次/末次时间；详情保存该指纹最新一次请求的完整上下文、认证头脱敏后的多值请求头、正文头尾快照、完整正文哈希、会话来源和压缩证据。正文快照最多 256KB、保留 3 天，基础记录保留 30 天。',
        refresh: '刷新记录',
        ...
        snapshotUnavailable: '该记录没有请求快照：可能创建于详情快照功能上线前，或快照已超过 3 天保留期。',
        requestBodyMetadata: '请求体元数据',
        detailBodyHint: '结构化元数据用于快速筛选；上方请求正文快照用于复核实际内容。明确或强特征命中的压缩请求只观察放行。',
        emptyLogs: '暂无请求管控记录',

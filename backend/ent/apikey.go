...
type APIKey struct {
	...
	// Hashed API key material used for authentication
	KeyHash *string `json:"key_hash,omitempty"`
	// Hash algorithm for key_hash
	KeyHashAlg string `json:"key_hash_alg,omitempty"`
	// Secret-independent SHA-256 lookup hash for API key authentication
	KeyLookupHash *string `json:"key_lookup_hash,omitempty"`
	// Non-secret prefix shown in UI and logs
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case apikey.FieldKey, apikey.FieldKeyHash, apikey.FieldKeyHashAlg, apikey.FieldKeyLookupHash, apikey.FieldKeyPrefix, apikey.FieldName, apikey.FieldStatus:
...
func (_m *APIKey) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case apikey.FieldKeyHashAlg:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field key_hash_alg", values[i])
			} else if value.Valid {
				_m.KeyHashAlg = value.String
			}
		case apikey.FieldKeyLookupHash:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field key_lookup_hash", values[i])
			} else if value.Valid {
				_m.KeyLookupHash = new(string)
				*_m.KeyLookupHash = value.String
			}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("key_hash_alg=")
	builder.WriteString(_m.KeyHashAlg)
	builder.WriteString(", ")
	if v := _m.KeyLookupHash; v != nil {
		builder.WriteString("key_lookup_hash=")
		builder.WriteString(*v)
	}
	builder.WriteString(", ")

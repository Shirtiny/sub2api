...
func (_c *UserCreate) AddCafeCouponIDs(ids ...int64) *UserCreate {
	_c.mutation.AddCafeCouponIDs(ids...)
	return _c
}

// AddCafeCoupons adds the "cafe_coupons" edges to the CafeCoupon entity.
func (_c *UserCreate) AddCafeCoupons(v ...*CafeCoupon) *UserCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddCafeCouponIDs(ids...)
}
...
func (_c *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.CafeCouponsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,

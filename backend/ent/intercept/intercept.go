...
type CafeCouponFunc func(context.Context, *ent.CafeCouponQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f CafeCouponFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.CafeCouponQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.CafeCouponQuery", q)
}

// The TraverseCafeCoupon type is an adapter to allow the use of ordinary function as Traverser.
type TraverseCafeCoupon func(context.Context, *ent.CafeCouponQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseCafeCoupon) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseCafeCoupon) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.CafeCouponQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.CafeCouponQuery", q)
}

...
type UserSubscriptionQuery struct {
	...
	withConcurrencyEntitlements *SubscriptionConcurrencyEntitlementQuery
...
// QueryConcurrencyEntitlements chains the current query on the "concurrency_entitlements" edge.
func (_q *UserSubscriptionQuery) QueryConcurrencyEntitlements() *SubscriptionConcurrencyEntitlementQuery {
...
func (_q *UserSubscriptionQuery) Clone() *UserSubscriptionQuery {
	...
	return &UserSubscriptionQuery{
		...
		withConcurrencyEntitlements: _q.withConcurrencyEntitlements.Clone(),
...
// WithConcurrencyEntitlements tells the query-builder to eager-load the nodes that are connected to
// the "concurrency_entitlements" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserSubscriptionQuery) WithConcurrencyEntitlements(opts ...func(*SubscriptionConcurrencyEntitlementQuery)) *UserSubscriptionQuery {
	query := (&SubscriptionConcurrencyEntitlementClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withConcurrencyEntitlements = query
	return _q
}
...
func (_q *UserSubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserSubscription, error) {
	var (
		...
		loadedTypes = [5]bool{
			...
			_q.withConcurrencyEntitlements != nil,
		}
	)
	...
	if query := _q.withConcurrencyEntitlements; query != nil {
		if err := _q.loadConcurrencyEntitlements(ctx, query, nodes,
			func(n *UserSubscription) { n.Edges.ConcurrencyEntitlements = []*SubscriptionConcurrencyEntitlement{} },
			func(n *UserSubscription, e *SubscriptionConcurrencyEntitlement) {
				n.Edges.ConcurrencyEntitlements = append(n.Edges.ConcurrencyEntitlements, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *UserSubscriptionQuery) loadConcurrencyEntitlements(ctx context.Context, query *SubscriptionConcurrencyEntitlementQuery, nodes []*UserSubscription, init func(*UserSubscription), assign func(*UserSubscription, *SubscriptionConcurrencyEntitlement)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*UserSubscription)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptionconcurrencyentitlement.FieldSubscriptionID)
	}
	query.Where(predicate.SubscriptionConcurrencyEntitlement(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(usersubscription.ConcurrencyEntitlementsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err

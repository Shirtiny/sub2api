...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	userRepository := repository.ProvideUserRepository(client, db, settingService)
	...
	billingCacheService := service.ProvideBillingCacheService(billingCache, userRepository, userSubscriptionRepository, apiKeyRepository, userRPMCache, userGroupRateRepository, configConfig, settingService, serviceUserPlatformQuotaRepository)
	...
	waitlistRepository := repository.NewWaitlistRepository(client)
	authService := service.NewAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, emailQueueService, promoService, subscriptionService, affiliateService, serviceUserPlatformQuotaRepository, waitlistRepository)
	...
	contentModerationService := service.ProvideContentModerationService(settingRepository, contentModerationRepository, contentModerationHashCache, groupRepository, userRepository, apiKeyAuthCacheInvalidator, emailService, settingService)
	requestControlRepository := repository.NewRequestControlRepository(db)
	requestControlService := service.ProvideRequestControlService(settingRepository, requestControlRepository, groupRepository, settingService, userRepository, apiKeyAuthCacheInvalidator, emailService)
	contentModerationHandler := admin.NewContentModerationHandler(contentModerationService, requestControlService)
	presaleNotificationRepository := repository.NewPresaleNotificationRepository(db)
	presaleNotificationService := service.NewPresaleNotificationService(presaleNotificationRepository, paymentConfigService, settingService, emailService, notificationEmailService)
	paymentHandler := admin.NewPaymentHandler(paymentService, paymentConfigService, presaleNotificationService)
	...
	gatewayHandler := handler.NewGatewayHandler(gatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, requestControlService)
	openAIGatewayHandler := handler.NewOpenAIGatewayHandler(openAIGatewayService, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, configConfig, requestControlService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, waitlistHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, idempotencyCoordinator, idempotencyCleanupService)

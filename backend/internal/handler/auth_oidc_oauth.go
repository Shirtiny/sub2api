...
func (k oidcJWK) publicKey() (any, error) {
	...
	case "EC":
		...
		if x.Sign() < 0 || y.Sign() < 0 || x.BitLen() > curve.Params().BitSize || y.BitLen() > curve.Params().BitSize {
			return nil, errors.New("ec point coordinate is out of range")
		}
		encodedPoint := make([]byte, 1+2*coordinateSize)
		encodedPoint[0] = 4 // SEC 1 uncompressed point encoding.

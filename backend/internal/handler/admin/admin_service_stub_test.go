...
func (s *stubAdminService) UpdateUserBalance(ctx context.Context, input service.UpdateUserBalanceInput) (*service.User, error) {
	user := service.User{ID: input.UserID, Balance: input.Balance, Status: service.StatusActive}
	return &user, nil
}

func (s *stubAdminService) UpdateUserMembershipPoints(ctx context.Context, userID int64, points float64, operation string, notes string) (*service.User, error) {
	user := service.User{ID: userID, TotalRecharged: points, Status: service.StatusActive}
	return &user, nil
}

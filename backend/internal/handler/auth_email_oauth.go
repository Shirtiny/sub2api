...
func (h *AuthHandler) completeEmailOAuthRegistration(c *gin.Context, provider string) {
	...
	sessionAffiliateCode := pendingSessionStringValue(session.UpstreamIdentityClaims, "aff_code")
	...
	if err != nil {
		_ = tx.Rollback()
		_ = h.authService.RollbackOAuthEmailAccountCreation(c.Request.Context(), user.ID, rollbackInvitationCode)
		response.ErrorFrom(c, err)
		return
	}

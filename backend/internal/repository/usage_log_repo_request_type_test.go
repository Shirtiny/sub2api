...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	t.Run("image_size_metadata_is_scanned", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			sql.NullInt64{},
			sql.NullInt64{},
			sql.NullInt64{Valid: true, Int64: 123},
			sql.NullString{},
			sql.NullString{},
			2,
			...
			sql.NullString{},
			sql.NullFloat64{},
			sql.NullString{}, // request_host
			now,
		}})
		require.NoError(t, err)
		require.NotNil(t, log.FirstByteMs)
		require.Equal(t, 123, *log.FirstByteMs)

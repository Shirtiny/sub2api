...
func TestForwardAlphaSearchAPIKeyAetherWireContract(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{
		"id":"search-session",
		"model":"gpt-5.6-sol",
		"prompt_cache_key":"must-not-forward",
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Header: http.Header{
			...
			"X-Aether-Upstream-Disposition": []string{"dispatched"},
		},
	...
	require.NotNil(t, result)
	require.Equal(t, 1, result.WebSearchCalls)
	require.Equal(t, "https://aether.example/v1/alpha/search", upstream.lastReq.URL.String())
	...
	require.Equal(t, "dispatched", recorder.Header().Get("x-aether-upstream-disposition"))
}

func TestForwardAlphaSearchAetherConflictDoesNotOuterFailoverOrBill(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"id":"stateful-session","model":"gpt-5.6-sol","commands":{"open":[{"ref_id":"turn0search0"}]}}`)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/alpha/search", bytes.NewReader(body))
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"error":{"code":"search_session_affinity_lost"}}`)),
	}}
	service := &OpenAIGatewayService{cfg: &config.Config{}, httpUpstream: upstream}
	account := &Account{
		ID:       10,
	...
	require.Contains(t, recorder.Body.String(), "search_session_affinity_lost")
}

func TestForwardAlphaSearchAPIKeyUnsupportedEndpointTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)

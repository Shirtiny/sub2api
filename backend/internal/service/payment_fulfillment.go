...
func (s *PaymentService) doBalance(ctx context.Context, o *dbent.PaymentOrder, lease *paymentFulfillmentLease) error {
	...
	redeemCtx := ContextRechargePointsFactor(ContextSkipRedeemAffiliate(ctx), paidRechargePointsFactor(o.Amount, o.PayAmount))
...
func (s *PaymentService) markCompleted(ctx context.Context, o *dbent.PaymentOrder, lease *paymentFulfillmentLease, auditAction string) error {
	...
	// The successful lease owner is the only worker that may send the
	// notification. Custom subscription fulfillment records its success audit
	// inside the assignment transaction, so notification dispatch cannot depend
	// on whether markCompleted had to create that audit entry.

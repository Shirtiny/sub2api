...
func TestCalculateProgress_LimitedQuotaEndsAtSubscriptionExpiry(t *testing.T) {
	svc := newTestSubscriptionService()
	now := time.Now()
	monthlyStart := now.Add(-45 * 24 * time.Hour)
	...
	group := &Group{Name: "Limited", MonthlyLimitUSD: ptrFloat64(100)}

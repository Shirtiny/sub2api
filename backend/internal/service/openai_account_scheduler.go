...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	account = s.service.recheckSelectedOpenAIAccountFromDB(ctx, req.GroupID, account, req.RequestedModel, req.RequireCompact, req.RequiredCapability)
	...
	// WaitPlan.MaxConcurrency 浣跨敤 Concurrency锛堥潪 EffectiveLoadFactor锛夛紝鍥犱负 WaitPlan 鎺у埗鐨勬槸 Redis 瀹為檯骞跺彂妲戒綅绛夊緟銆?
...
func (h openAIAccountCandidateHeap) Less(i, j int) bool {
	// 鏈€灏忓爢鏍硅妭鐐逛繚瀛樷€滄渶宸€濆€欓€夛紝渚夸簬 O(log k) 缁存姢 topK銆?
...
func deriveOpenAISelectionSeed(req OpenAIAccountScheduleRequest) uint64 {
	...
	seed := hasher.Sum64()
	// 瀵光€滄棤浼氳瘽閿氱偣鈥濈殑绾礋杞藉潎琛¤姹傚紩鍏ユ椂闂寸喌锛岄伩鍏嶅浐瀹氬懡涓悓涓€璐﹀彿銆?
...
func buildOpenAIWeightedSelectionOrder(
	...
	for i := range pool {
		// 灏?top-K 鍒嗗€煎钩绉诲埌姝ｅ尯闂达紝閬垮厤鈥滃崟涓€鏈€楂樺垎璐﹀彿鈥濋暱鏈熷瀯鏂€?
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrder(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		fresh = s.service.recheckSelectedOpenAIAccountFromDB(ctx, req.GroupID, fresh, req.RequestedModel, false, req.RequiredCapability)
		if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport) || !s.isAccountRequestCompatible(ctx, fresh, req) {
			continue
		}
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		...
		// require_privacy_set: 璺宠繃 privacy 鏈缃殑璐﹀彿骞舵爣璁板紓甯?
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatible(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) bool {
	...
	// rechecks won't reach healthy accounts that fell outside TopK 鈥?manifesting as
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImages(
	...
	// 濡傛灉瑕佹眰 native 鑳藉姏锛堝鎸囧畾浜嗘ā鍨嬶級浣嗘病鏈夊彲鐢ㄧ殑 APIKey 璐﹀彿锛屽洖閫€鍒?basic锛圤Auth 璐﹀彿锛?

...
func (s *EmailService) buildVerifyCodeEmailBody(code, siteName string) string {
	return renderEmailCard(siteName, notificationEmailDefaultLocale, "Email verification code",
		`<p>Your verification code is:</p>`+emailVerificationCode(code)+`
...
func (s *EmailService) buildPasswordResetEmailBody(resetURL, siteName string) string {
	if !isSafeNotificationEmailURL(resetURL) {
		resetURL = ""
	}
	link := html.EscapeString(resetURL)
	return renderEmailCard(siteName, notificationEmailLocaleChinese, "密码重置请求", `

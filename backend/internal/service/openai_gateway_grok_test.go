...
func TestGrokPoolPassthroughUsesConfiguredAetherResponsesEndpoint(t *testing.T) {
	...
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{"model":"grok-4.5"}`))
	...
	req, err := svc.buildUpstreamRequestOpenAIPassthrough(c.Request.Context(), c, account, []byte(`{"model":"grok-4.5"}`), "aether-key")
	require.NoError(t, err)
	require.Equal(t, "https://aether.example/v1/responses", req.URL.String())
	require.Equal(t, "Bearer aether-key", req.Header.Get("Authorization"))
}

func TestGrokPoolPassthroughMapsResponsesAliasBeforeAether(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"grok-latest","input":"hello"}`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(body))
	account := &Account{
		ID:          91,
		Platform:    PlatformGrok,
...
func TestGrokPoolResponsesMarksConfiguredAuthErrorForSameAccountRetry(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"error":{"message":"pool key rotated"}}`)),
	}}
	svc := &OpenAIGatewayService{cfg: &config.Config{}, httpUpstream: upstream}
...
func TestGrokPoolPassthroughRequiresExplicitPoolMode(t *testing.T) {
	account := &Account{
		Platform: PlatformGrok,
		Type:     AccountTypeAPIKey,
		Credentials: map[string]any{
			"api_key":  "direct-xai-key",
			"base_url": "https://api.x.ai/v1",
		},
	}
	require.False(t, account.IsGrokPoolPassthrough())
}

func TestGrokPoolPassthroughUsesAetherChatAndMessagesEndpoints(t *testing.T) {
	t.Setenv(xai.EnvAllowUnsafeURLOverrides, "true")
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", strings.NewReader(`{"model":"grok-4.5"}`))
	account := &Account{
		Platform: PlatformGrok,
		Type:     AccountTypeAPIKey,
		Credentials: map[string]any{
	...
	require.NoError(t, err)
	require.Equal(t, "https://aether.example/v1/chat/completions", chatURL)
	req, err := svc.buildUpstreamRequestOpenAIMessagesPassthrough(c.Request.Context(), c, account, []byte(`{"model":"grok-4.5"}`), "aether-key")
	require.NoError(t, err)
	require.Equal(t, "https://aether.example/v1/messages", req.URL.String())
	require.Equal(t, "Bearer aether-key", req.Header.Get("Authorization"))
}

func TestGrokPoolPassthroughUsesAetherMediaEndpoints(t *testing.T) {
	account := &Account{
		Platform: PlatformGrok,
		Type:     AccountTypeAPIKey,
		Credentials: map[string]any{
